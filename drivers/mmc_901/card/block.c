...
static int sdmmc_blk_issue_rw_rq(struct mmc_queue *mq, struct request *req)
{
	 ...
	 */
	//mmc_debug_ms("brq->cmd.resp[0]:%#x, line:%d\n", brq->cmd.resp[0], __LINE__);
    ...
    //delete all retry code. modifyed by xbw at 2011-11-17
		...
		if (brq->cmd.resp[0] & CMD_ERRORS) {
			pr_err("%s:%d  %s: r/w command failed, status = %#x\n", __FUNCTION__, __LINE__,
...
static int sdmmc_blk_issue_rq(struct mmc_queue *mq, struct request *req)
{
	...
	//mmc_debug_ms("%s  line:%d, req->cmd_flags:%#x\n", __FUNCTION__, __LINE__, req->cmd_flags);
	if (req && req->cmd_flags & REQ_DISCARD) {
		if (req->cmd_flags & REQ_SECURE)
			ret = mmc_blk_issue_secdiscard_rq(mq, req);
		else
			ret = mmc_blk_issue_discard_rq(mq, req);
		//mmc_debug_ms("%s  line:%d\n", __FUNCTION__, __LINE__);
