...
#define nwddbg() printk("=======%d %s ==========\n",__LINE__,__FUNCTION__)
#define RK29_SDMMC_xbw_Debug 0
#define CONFIG_USE_SDMMC0_FOR_WIFI_DEVELOP_BOARDwj 1
			 ...
			 const char *buf, size_t count)
{
    ...
    */
    #if !defined(CONFIG_USE_SDMMC0_FOR_WIFI_DEVELOP_BOARDwj)
...
int rk29_sdmmc_change_clk_div(struct rk29_sdmmc *host, u32 freqHz)
{
    ...
    if(host->old_div != div)
    {
        printk(KERN_INFO "%s..%d..  aa newDiv=%u, newCLK=%uKhz [%s]\n", \
...
static irqreturn_t rk29_sdmmc_interrupt(int irq, void *dev_id)
{
	...
	bool present_old;
	int sdio_irq=0;
  ///if(host->host_dev_id==1) printk("++++++++++++++++++++%d+++%d++++++++++++++++++++++++++++++++\n",__LINE__,host->host_dev_id);
	spin_lock(&host->lock);
    ...
    if(pending & SDMMC_INT_CD) 
    {
		...
		mmc_debug_ms("pin3c6:%d, present:%d, present_old:%d \n", gpio_get_value(RK30_PIN3_PC6), present, present_old);
    	if(present != present_old)
    	{    	    
			printk(KERN_INFO "INT_CD=0x%x, INT-En=%d, hostState=%d, present Old=%d ==> New=%d [%s]\n", pending, host->mmc->re_initialized_flags, host->state, present_old, present,  host->dma_name);
...
nwddbg();
   ...
   if((RK29_CTRL_SDMMC_ID == host->host_dev_id)|| (RK29_SDMMC_EMMC_ID == host->host_dev_id)) 
    ...
    {	nwddbg();
        ...
        {
            #if defined(CONFIG_USE_SDMMC1_FOR_WIFI_DEVELOP_BOARD)
                if(rk29_sdmmc_get_cd(host->mmc)) //(1)//test
                {
                    set_bit(RK29_SDMMC_CARD_PRESENT, &host->flags);
                }
                else
	...
	nwddbg();
///clk_set_rate(host->clk,30000000);
///clk_enable(host->clk);
	mmc_add_host(mmc);
