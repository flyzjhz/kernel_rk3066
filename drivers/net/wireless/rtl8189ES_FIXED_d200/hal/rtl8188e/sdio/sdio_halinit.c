...
*/
	...
	//	printk("++++++++%d %s++++++VAL_WJ=%x+++++\n",__LINE__,__FUNCTION__,value16);
//		printk("++++++++%d %s++++++VAL_WJ=%x+++++\n",__LINE__,__FUNCTION__,rtw_read16(padapter, REG_CR));
	...
	)
{
	...
	if (1)///(AutoLoadFail)
	{
         	//sMacAddr[5] = (u1Byte)GetRandomNumber(1, 254); ///raw ver,this line is commetoff ,wj add this ,but this func cannot use
		
		for (i=0; i<6; i++)
			pEEPROM->mac_addr[i] = sMacAddr[i];
		/***/
	u32	curtime = rtw_get_current_time(); ///wj add
		pEEPROM->mac_addr[3] = (u8)(curtime & 0xff) ;
		pEEPROM->mac_addr[4] = (u8)((curtime>>8) & 0xff) ;
		pEEPROM->mac_addr[5] = (u8)((curtime>>16) & 0xff) ;  
		_rtw_memcpy( &hwinfo[EEPROM_MAC_ADDR_88ES], pEEPROM->mac_addr,ETH_ALEN);
	}
	else
	{
...
///wj
	if(1)///( (_rtw_memcmp(pEEPROM->mac_addr, null_mac_addr, ETH_ALEN)) ||	(_rtw_memcmp(pEEPROM->mac_addr, multi_mac_addr, ETH_ALEN)) ) 
	{
	...
	printk("/n/n/n  Hal_ReadMACAddrFromFile_8188ES: Permanent Address = %02x-%02x-%02x-%02x-%02x-%02x\n",
		  pEEPROM->mac_addr[0], pEEPROM->mac_addr[1],
		  pEEPROM->mac_addr[2], pEEPROM->mac_addr[3],
		  pEEPROM->mac_addr[4], pEEPROM->mac_addr[5]);
}
...
///	pEEPROM->bautoload_fail_flag = TRUE; ///wj ADD THIS LINE
#ifdef CONFIG_EFUSE_CONFIG_FILE
