...
extern unsigned char UdiskInsertFlag;
struct usb_hub {
...
extern unsigned char FristStartFlag;//add by lusterzhang
static int hub_probe(struct usb_interface *intf, const struct usb_device_id *id)
{
	...
	hdev = interface_to_usbdev(intf);
	 mdelay(50);
        if(FristStartFlag==0x55)
        {
                mdelay(200);//add by lusterzhang
        }
...
void usb_disconnect(struct usb_device **pdev)
{
	 ...
	 */
	usb_set_device_state(udev, USB_STATE_NOTATTACHED);
	dev_info(&udev->dev, "aaaaa USB disconnect, device number %d\n",
			udev->devnum);
	UdiskInsertFlag=0;
					...
					u16 portstatus, u16 portchange)
{
	...
	struct usb_device *udev;
	int status, i;
	printk("lusterzhang hub_port_connect_change...\n");
	dev_dbg (hub_dev,
...
static void hub_events(void)
{
	...
	while (1) {
		...
		if (list_empty(&hub_event_list)) {
			printk("lusterzhang hub_events empty...\n");
			spin_unlock_irq(&hub_event_lock);
			break;
		}
		printk("lusterzhang hub_events...\n");
		...
		for (i = 1; i <= hub->descriptor->bNbrPorts; i++) {
			...
			if (connect_change)
			{
				printk("lusterzhang call hub_port_connect_change start...\n");
				hub_port_connect_change(hub, i,
						portstatus, portchange);
				printk("lusterzhang call hub_port_connect_change end...\n");
			}
		} /* end for i */
...
{
	printk("lusterzhang hub_disconnect_device...\n");
    	hub_port_connect_change(hub, 1, 0, 0x2);
}
unsigned char mUsbDongleFlag=0x55;
static int hub_thread(void *__unused)
{
	...
	mUsbDongleFlag=0x55;
	msleep(4000);
	printk("lusterzhang hub_thread start...\n");
	do {
		hub_events();
...
static int usb_reset_and_verify_device(struct usb_device *udev)
{
	...
	int 				i, ret = 0;
	int				port1 = udev->portnum;
	printk("lusterzhang usb_reset_and_verify_device...\n");
	if (udev->state == USB_STATE_NOTATTACHED ||
			udev->state == USB_STATE_SUSPENDED) {
