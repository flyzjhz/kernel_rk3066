...
static struct miscdevice rk29_wdt_miscdev = {
	.minor		= MISC_DYNAMIC_MINOR,///WATCHDOG_MINOR,
	.name		= "nwdwatchdog",
	.fops		= &rk29_wdt_fops,
};
...
EXPORT_SYMBOL(rk29_wdt_keepalive);
/* device interface */
...
static int __devinit rk29_wdt_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR(wdt_clock)) {	printk(" %s %d\n",__FUNCTION__,__LINE__);
	...
	printk(" %s %d\n",__FUNCTION__,__LINE__);
	rk29_wdt_start();return 0; ///wj add
...
static char banner[] __initdata =
	"\nRK29 Watchdog Timer, (c) 2011 Rockchip Electronics\n";
