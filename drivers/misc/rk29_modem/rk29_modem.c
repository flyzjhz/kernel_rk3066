...
#if 0
#define nwd_dbg()  printk("===============%d %s========",__LINE__,__FUNCTION__);
#else 
#define nwd_dbg() do{ } while(0)
#endif
...
int rk29_modem_change_status(struct rk29_modem_t *rk29_modem, int status)
{
	...
	gpio_direction_output(RK30_PIN4_PD1, 0);   ///
	nwd_dbg();
        break;
    case MODEM_ENABLE :
        rk29_modem_turnon(rk29_modem->modem_power, 1);
	gpio_direction_output(RK30_PIN4_PD1, 1);   ///
	nwd_dbg();
        break;
    case MODEM_SLEEP:
        ret = -1;
...
#endif
{
    ...
    if(new_state == rk29_modem->status) {nwd_dbg(); 
     	///return _count;
    }
...
// 对modem初始化
	...
	nwd_dbg();
    g_rk29_modem = rk29_modem;  nwd_dbg();

    return 0;

failed_device_init:
    platform_driver_unregister(rk29_modem->driver);nwd_dbg();

failed_create_class:
    class_destroy(rk29_modem_class); nwd_dbg();
