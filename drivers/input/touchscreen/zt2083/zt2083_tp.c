...
unsigned char pen_count     = 0;
int data_x_tmp[DATA_NUM] = {0};
int data_y_tmp[DATA_NUM] = {0};
unsigned char SendKeybroadOutFlag=0;
...
static void key_timer(unsigned long data)
{
	...
	if(SendKeybroadOutFlag)
	{
		SendKeybroadOutFlag=0;
...
unsigned int ProPointx=0;
unsigned int ProPointy=0;
extern int ts_linear_scale_cal(int *x, int *y);
...
/*
        ...
        tmpx= zt2803_ts->x_rp;
        tmpy= zt2803_ts->y_rp;
	ts_linear_scale_cal(&tmpx,&tmpy);
	zt2803_ts->x_rp = tmpx;
	zt2803_ts->y_rp = tmpy;
        printk("3333 after cali ----->  x: %-4d, y: %-4d \n", zt2803_ts->x_rp, zt2803_ts->y_rp);
*/
#ifndef ZT2083
	zt2803_ts->x_rp = zt2803_ts->x_rp - 50;
	zt2803_ts->x_rp = 800 * zt2803_ts->x_rp / 3920;

	zt2803_ts->y_rp = zt2803_ts->y_rp - 190;
	zt2803_ts->y_rp = 480 * zt2803_ts->y_rp / 3730;	
#else
        zt2803_ts->x_rp = zt2803_ts->x_rp - 50;
	zt2803_ts->x_rp = 800 * zt2803_ts->x_rp / 3920;
...
*/
        mod_timer(&gtp_key_timer, jiffies + msecs_to_jiffies(80));
	SendKeybroadOutFlag=1;
}
