...
static int i2c_read_bytes( struct i2c_client *client, u8 *buf, int len )
{
	...
	if (ret != len) {
	    TPD_DMESG(TPD_DEVICE " fail to read %d bytes : %d\n", len, ret);
	    return ret;
	}
    } else {
...
static int i2c_write_bytes( struct i2c_client *client, u8 *buf, int len )
{
	...
	if (ret != len) {
	    TPD_DMESG(TPD_DEVICE " fail to write %d bytes : %d\n", len, ret);
	    return ret;
	}
    } else {
	for (i = 0; i < len; i++) {
...
{             
    ...
    if (err) {
        TPD_DMESG(TPD_DEVICE " Fail to read FW CRC info %d\n", err );
    } else {
        TPD_DMESG(TPD_DEVICE " FW CRC :  %x %x\n", tpd_fw_crc[0], tpd_fw_crc[1] );
    }
    #endif
...
static long pixcir_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
    ...
    case POINTS_VALUE:		//POINTS_VALUE = 20
	client->addr = SLAVE_ADDR;
	status_reg = POINTS_VALUE;
	break;
...
static ssize_t pixcir_read (struct file *file, char __user *buf, size_t count, loff_t *offset)
{
    ...
    {
    case BOOTLOADER_STU:
	err = i2c_read_bytes(client, bootloader_stu, sizeof(bootloader_stu));
	if (err) {
		dev_err(&client->dev,
			"%s: BOOTLOADER_STU: i2c_read_bytes() failed, err=%d\n",
			__func__, err);
		return -EFAULT;
	}
    ...
    case POINTS_VALUE:
	if(copy_to_user(buf, g_buffer, sizeof(g_buffer))) {
		dev_err(&client->dev,
			"%s: POINTS_VALUE: copy_to_user() failed.\n", __func__);
		return -EFAULT;
	} else {
		ret = sizeof(g_buffer);
	}
	...
	err = i2c_read_bytes(client, tmp, count);
	if (err) {
		dev_err(&client->dev,
			"%s: default: i2c_read_bytes() failed, err=%d\n",
			__func__, err);
		return -EFAULT;
	}
...
static ssize_t pixcir_write(struct file *file, const char __user *buf, size_t count, loff_t *ppos)
{
	...
	err = i2c_write_bytes(client, tmp, count);
	if (err) {
		dev_err(&client->dev,
			"%s: CALIBRATION: i2c_send_vytes() failed, err=%d\n",
			__func__, err);
		kfree(tmp);
		return -EFAULT;
	} else {
	...
	err = i2c_write_bytes(client, bootload_data, count);
	if (err) {
		dev_err(&client->dev,
			"%s: BOOTLOADER: i2c_send_bytes() failed, err = %d\n",
			__func__, err);
		return -EFAULT;
	} else {
		ret = count;
   ...
   default:
	...
	if (err) {
		dev_err(&client->dev,
			"%s: default: i2c_sned_bytes() failed, err=%d\n",
			__func__, err);
		kfree(tmp);
		return -EFAULT;
	} else {
...
{
    u8 mode[2] = { TPD_POWER_MODE_REG, 0x03 }; //0x01--Idle mode; 0xA4--Idle mode automatically switch; 0x03--Sleep mode; 
    ...
    i2c_write_bytes( i2c_client, mode, 2 );
    tpd_halt = 1;
...
{
    int ret;
    TPD_DMESG("MediaTek tangoc32 touch panel driver init\n");
