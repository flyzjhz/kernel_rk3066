...
#ifdef CONFIG_YINGLONG_ZHENGJI_963A_1024600
static struct timer_list Timer_keyboard_power;
static struct timer_list Timer_keyboard_mute;
static unsigned int long_pressed = 0;
static unsigned int timecnt = 1;
static unsigned char mKeyFlag = 0;
unsigned long time_prev = 0;
unsigned int press_dou = 0;
unsigned int short_pressed = 1;
unsigned int long_press_time;

int touch_key_code[] = {KEY_DISPLAYTOGGLE_NWD, KEY_HOME, NAVI_NWD, KEY_VOLUMEUP_NWD, KEY_VOLUMEDOWN_NWD, KEY_POWEROFF_NWD};
#else
int touch_key_code[] = {NAVI_NWD, KEY_HOME, KEY_VOLUMEUP_NWD, KEY_VOLUMEDOWN_NWD};
#endif
//int touch_key_code[] = {NAVI_NWD, KEY_HOME,KEY_BACK,KEY_VOLUMEDOWN,KEY_VOLUMEUP,KEY_VOLUMEDOWN_NWD, KEY_VOLUMEUP_NWD};
int touch_key_press[] = {0, 0, 0, 0, 0, 0, 0, 0};
...
#else
#define KEYPAD01_X1	200
#define KEYPAD01_X2	300
#define KEYPAD02_X1	460
#define KEYPAD02_X2	560
#define KEYPAD03_X1	720
#define KEYPAD03_X2	820
#define KEYPAD04_X1	980
#define KEYPAD04_X2	1080
#define KEYPAD05_X1	1240
#define KEYPAD05_X2	1340
#define KEYPAD_Y	2100
...
void nwd_calibration(struct ilitek_ts_item *panel, int *x, int *y)
{
	int px = 0, py = 0;
	...
	int max_y)
{
	...
	printk("----------> i2c.max_x = %d, i2c.max_y = %d\n", max_x, max_y);
	#ifndef ROTATE_FLAG
		...
		for(i = 0; i < max_point; i++){
			...
			if(tp_status){
				if(i2c.keyflag == 0){
					for(j = 0; j <= i2c.keycount; j++){
						if((x >= i2c.keyinfo[j].x && x <= i2c.keyinfo[j].x + i2c.key_xlen) && (y >= i2c.keyinfo[j].y && y <= i2c.keyinfo[j].y + i2c.key_ylen)){
							...
							release_flag[0] = 1;
							printk("Key, Keydown ID=%d, X=%d, Y=%d, key_status=%d,keyflag=%d\n", i2c.keyinfo[j].id ,x ,y , i2c.keyinfo[j].status,i2c.keyflag);
							break;
						}
					}
				}
				if(touch_key_hold_press == 0){
					...
					printk("--->Point, ID=%02X, X=%04d, Y=%04d,release_flag[%d]=%d,tp_status=%d,keyflag=%d\n",i, x,y,i,release_flag[i],tp_status,i2c.keyflag);	
				}
				if(touch_key_hold_press == 1){
					for(j = 0; j <= i2c.keycount; j++){
						if((i2c.keyinfo[j].status == 1) && (x < i2c.keyinfo[j].x || x > i2c.keyinfo[j].x + i2c.key_xlen || y < i2c.keyinfo[j].y || y > i2c.keyinfo[j].y + i2c.key_ylen)){
							...
							printk("Key, Keyout ID=%d, X=%d, Y=%d, key_status=%d\n", i2c.keyinfo[j].id ,x ,y , i2c.keyinfo[j].status);
							break;
						}
					}
				}
				...
				release_flag[i] = 0;
				printk("Point, ID=%02X, X=%04d, Y=%04d,release_flag[%d]=%d,tp_status=%d\n",i, x,y,i,release_flag[i],tp_status);	
				input_mt_sync(i2c.input_dev);
			} 
				...
				if (touch_key_hold_press == 1){
					for(i = 0; i < i2c.keycount; i++){
						if(i2c.keyinfo[i].status){
							...
							touch_key_hold_press = 0;
							printk("Key, Keyup ID=%d, X=%d, Y=%d, key_status=%d, touch_key_hold_press=%d\n", i2c.keyinfo[i].id ,x ,y , i2c.keyinfo[i].status, touch_key_hold_press);
						}
					}
				}
			}
			printk("release_counter=%d,packet=%d\n",release_counter,packet);
		}
	}
	// multipoint process
				...
				switch ((mult_tp_id & 0xC0)){
					...
					case RELEASE_KEY:
						...
						for(key=0; key<sizeof(touch_key_code)/sizeof(touch_key_code[0]); key++){
							if(touch_key_press[key]){
								if(touch_key_code[key] == KEY_HOME)
								{	
									home_key_to0();
									//printk("%s key release, %X, %d, %d\n", __func__, buf[0], x, y);
								}
							#ifdef CONFIG_YINGLONG_ZHENGJI_963A_1024600
								else if(touch_key_code[key] == KEY_DISPLAYTOGGLE_NWD)
								{
									long_pressed = 0;
								}
							#endif
					...
					case TOUCH_KEY:
						...
						if (x > KEYPAD01_X1 && x<KEYPAD01_X2)		// btn 1
							...
							key_id=3;
						else if (x > KEYPAD05_X1 && x<KEYPAD05_X2)	// btn 5
							key_id=4;
						else 
							key_flag=0;
						#endif
						//printk("touch_key_press[%d] = %d, touch_key_hold_press %d, keyflag = %d x = %d\n", key_id, touch_key_press[key_id], touch_key_hold_press, key_flag, x);
						if((touch_key_press[key_id] == 0) && (touch_key_hold_press == 0 && key_flag)){
							if(touch_key_code[key_id] == KEY_HOME)	
							{
								home_key_to1();
							#ifdef CONFIG_YINGLONG_ZHENGJI_963A_1024600
								home_key_to0();//cancel 963A KEY_HOME touch_key long_press function.
							#endif
								//printk("Key:%d ID:%d press x=%d,touch_key_hold_press=%d,key_flag=%d\n", touch_key_code[key_id], key_id,x,touch_key_hold_press,key_flag);
							}
						#ifdef CONFIG_YINGLONG_ZHENGJI_963A_1024600
							else if(touch_key_code[key_id] == KEY_DISPLAYTOGGLE_NWD)
							{
								mod_timer(&Timer_keyboard_power, jiffies + msecs_to_jiffies(1));
							}
						#endif
							else
							{
					...
					case TOUCH_POINT:
						...
						#if defined(CONFIG_YINGLONG_YUANCHEPIN_800480)	
						x = (800 * x) / 3970;						
						y = y - 20;
						y = (480 * y) / 2700;
						y = 480 - y;
						x = 800 - x;
						#elif defined(CONFIG_YINGLONG_ZHENGJI_1024600)
						x = (x*1024)/3684;
						y = (y*600)/2542;
						if(x>1024) x=1024;
						if(y>600) y=600;
						#elif defined(CONFIG_YINGLONG_YUANCHEPIN_NX300_800480)
						nwd_calibration(&yl_nx300, &x, &y);
						#elif defined(CONFIG_YINGLONG_ZHENGJI_FOCUS_1024600)
						x = 4096 - x;
						y = 3072 - y;
						#elif defined(CONFIG_YINGLONG_ZHENGJI_963A_1024600)
						x = (x*1024)/16384;
						y = (y*600)/9600;
						if(x>1024) x=1024;
						if(y>600) y=600;
...
#ifdef CONFIG_YINGLONG_ZHENGJI_963A_1024600
static void timer_callbackmute(unsigned long data)
{
	struct input_dev *input = i2c.input_dev;

	if (jiffies_to_msecs (jiffies - time_prev) >160)
	{
		input_report_key(input, KEY_DISPLAYTOGGLE_NWD, 1);
//		input_report_key(input, touch_key_code[0], 1);
		input_sync(input);

		input_report_key(input, KEY_DISPLAYTOGGLE_NWD, 0);
//		input_report_key(input, touch_key_code[0], 0);
		input_sync(input);
//		printk("=======press mute!=======\n");
		short_pressed = 1;
		mKeyFlag = 0;
...
static void timer_callbackpower(unsigned long data)
{
	struct input_dev *input = i2c.input_dev;
	...
	mod_timer(&Timer_keyboard_power, jiffies + msecs_to_jiffies(20));

	if (timecnt != 1  && jiffies_to_msecs (jiffies - time_prev) < 50 )
	{
		press_dou = 1;
	...
	press_dou = 0;
//	printk("long_pressed:%d, mKeyFlag:%d\n", long_pressed, mKeyFlag);
//	if(mKeyFlag>2&&mKeyFlag < 20 &&key_released!= 1 && long_pressed !=1)
	if(mKeyFlag < 20 && long_pressed != 1)
	{
		mod_timer(&Timer_keyboard_mute, jiffies + msecs_to_jiffies(200));
		timecnt++;
		time_prev = jiffies;
	...
	if(mKeyFlag==20)
	{
//		input_report_key(input, touch_key_code[5], 1);
		input_report_key(input, KEY_POWEROFF_NWD, 1);
		input_sync(input);
		mKeyFlag = 0;
//		input_report_key(input, touch_key_code[5], 0);
		input_report_key(input, KEY_POWEROFF_NWD, 0);
		input_sync(input);
		long_pressed = 1;
	...
	void)
{
	...
	setup_timer( &Timer_keyboard_power, timer_callbackpower, 0 );
	setup_timer( &Timer_keyboard_mute, timer_callbackmute, 0 );
	if(ret == 0){
		...
		printk("i2c.max_tp = %d, i2c.max_x = %d, i2c.max_y = %d\n", i2c.max_tp, i2c.max_x, i2c.max_y);
#if defined(CONFIG_YINGLONG_YUANCHEPIN_800480) || defined(CONFIG_YINGLONG_YUANCHEPIN_NX300_800480)
	    i2c.max_x = 800;	
		i2c.max_y = 480;
#elif (defined(CONFIG_YINGLONG_ZHENGJI_1024600) || defined(CONFIG_YINGLONG_ZHENGJI_963A_1024600))
	    i2c.max_x = 1024;	
		i2c.max_y = 600;
#endif
