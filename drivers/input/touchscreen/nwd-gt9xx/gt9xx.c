...
static struct proc_dir_entry *proc_entry_set_ts = NULL;
#endif
//********** For GT9XXF End **********//

static int print_coordinate = 0;
...
/*初始化的时候用到*/
struct nwd_ts_panel *init_panel = NULL;
/*****************************************************
config paramater through parameter
*****************************************************/
struct nwd_ts_panel *set_panel(int index)
{
	int i;
	for(i = 0; nwd_panel[i] != NULL; i++)	
	{
		if(nwd_panel[i]->index == index)
		{
			return nwd_panel[i];
		}
	}
...
extern void home_key_to1();
extern void home_key_to0();

//判断是否是按键并且是哪个按键
static int handle_key(struct goodix_ts_data* ts, int x, int y)
{
	int i = 0;

	for(i = 0; ts->nwd_ts_panel->key[i].key_range != 0xff; i++)
	{	
		//判断是否在按键区域
		if( x <= ts->nwd_ts_panel->key[i].key_x + ts->nwd_ts_panel->key[i].key_range && \
			x >= ts->nwd_ts_panel->key[i].key_x - ts->nwd_ts_panel->key[i].key_range &&  \
			y >= ts->nwd_ts_panel->key[i].key_y - ts->nwd_ts_panel->key[i].key_range &&  \
			y <= ts->nwd_ts_panel->key[i].key_y + ts->nwd_ts_panel->key[i].key_range )
		{
			if(ts->nwd_ts_panel->keyindex < 0)
			{
				//printk("f key: x:%d,y:%d,key[%d]:%d\n",x, y, i, ts->nwd_ts_panel->key[i].short_keyvalue);
				ts->nwd_ts_panel->key[i].down_jiffies = jiffies;
				ts->nwd_ts_panel->keyindex = i;
				return i;
			}
			else if(ts->nwd_ts_panel->keyindex == i)
			{
				//printk("o key: x:%d,y:%d,key[%d]:%d\n",x, y, i, ts->nwd_ts_panel->key[i].short_keyvalue);
				ts->nwd_ts_panel->key[i].total_jiffies = jiffies - ts->nwd_ts_panel->key[i].down_jiffies;
				return -2;
			}
			else 
			{
				//printk("d key: x:%d,y:%d,key[%d]:%d\n",x, y, i, ts->nwd_ts_panel->key[i].short_keyvalue);
				return -3;
			}
		}
	}
...
//定时器：抬起后才会上报键值，会分三种情况，一般按键，不带长按键的，就会在释放的时候才发键值
static void key_timer(unsigned long data)
{
	struct goodix_ts_data* ts = (struct goodix_ts_data *)data;
	if(ts->nwd_ts_panel->key[ts->nwd_ts_panel->keyindex].report_status != 1)
	{
		if( ts->nwd_ts_panel->key[ts->nwd_ts_panel->keyindex].short_keyvalue == KEY_HOME )
		{
			home_key_to1();
			home_key_to0();
		}
                else if(ts->nwd_ts_panel->key[ts->nwd_ts_panel->keyindex].short_keyvalue \
                                == ts->nwd_ts_panel->key[ts->nwd_ts_panel->keyindex].long_keyvalue)
		{
			input_event(ts->input_dev, EV_KEY, ts->nwd_ts_panel->key[ts->nwd_ts_panel->keyindex].long_keyvalue, 0);
			input_sync(ts->input_dev);
		}
		else		
		{
			input_event(ts->input_dev, EV_KEY, ts->nwd_ts_panel->key[ts->nwd_ts_panel->keyindex].short_keyvalue, 1);
			input_sync(ts->input_dev);
			input_event(ts->input_dev, EV_KEY, ts->nwd_ts_panel->key[ts->nwd_ts_panel->keyindex].short_keyvalue, 0);
			input_sync(ts->input_dev);
		}
	}

	ts->nwd_ts_panel->key[ts->nwd_ts_panel->keyindex].total_jiffies = 0;
	ts->nwd_ts_panel->key[ts->nwd_ts_panel->keyindex].down_jiffies = 0;
	ts->nwd_ts_panel->key[ts->nwd_ts_panel->keyindex].report_status = 0;
	ts->nwd_ts_panel->keyindex = -1;

	return;
}

//static void nwd_input_event(struct goodix_ts_data *ts, int key)
...
#endif
	if(print_coordinate)
		printk("\n\nsrc -> ID:%d, X:%d, Y:%d, W:%d\n", id, x, y, w); //调试触摸需要触摸，矫正前。主要是用于获取l_x, r_x, up_y, down_y.

	if(ts->nwd_ts_panel->mintomax)
	{
		ts->nwd_ts_panel->mintomax(ts->nwd_ts_panel, &x, &y);
	}

	if(print_coordinate)
    		printk("cdt -> ID:%d, X:%d, Y:%d, W:%d\n", id, x, y, w); //调试触摸需要,矫正后的坐标，用于获得真实的按键坐标

	if(ts->nwd_ts_panel->pixel_y < y  \
		|| y < 0 \
		|| ts->nwd_ts_panel->pixel_x < x \
		|| x < 0)
	{
		ret = handle_key(ts, x, y);
		if(-1 == ret)
		{
			return;
		}

		if(ts->nwd_ts_panel->key[ts->nwd_ts_panel->keyindex].long_keyvalue)
		{

			if(ts->nwd_ts_panel->key[ts->nwd_ts_panel->keyindex].short_keyvalue \
				== ts->nwd_ts_panel->key[ts->nwd_ts_panel->keyindex].long_keyvalue)
			{
				input_event(ts->input_dev, EV_KEY, ts->nwd_ts_panel->key[ts->nwd_ts_panel->keyindex].long_keyvalue, 1);
				input_sync(ts->input_dev);
			}
			else
			{
				if( ts->nwd_ts_panel->key[ts->nwd_ts_panel->keyindex].total_jiffies > 200 
					&& ts->nwd_ts_panel->key[ts->nwd_ts_panel->keyindex].report_status != 1)  
				{
					input_event(ts->input_dev, EV_KEY, ts->nwd_ts_panel->key[ts->nwd_ts_panel->keyindex].long_keyvalue, 1);
					input_sync(ts->input_dev);
		...
		gtp_key_timer.data = (unsigned long)ts;
		mod_timer(&gtp_key_timer, jiffies + msecs_to_jiffies(50));
		return;	
	}
...
void gtp_reset_guitar(struct i2c_client *client, s32 ms)
{
    ...
    GTP_DEBUG_FUNC();
    printk("func: %s : gt9xx Guitar reset\n", __func__);
...
static s32 gtp_init_panel(struct goodix_ts_data *ts)
{
    ...
    if(ts->nwd_ts_panel->config){
		printk("\nuse config : %s \n", ts->nwd_ts_panel->name);
		send_cfg_buf[0] = ts->nwd_ts_panel->config;
    }else{
		printk("\n\n\nuse default config paramater, make sure ..........\n\n");
	}
	...
	if(ts->nwd_ts_panel)
	{
		if(ts->nwd_ts_panel->int_trigger == 0)
		{
        		config[TRIGGER_LOC] &= 0xfe; 
		}
		else if(ts->nwd_ts_panel->int_trigger == 1)
		{
...
s32 gtp_read_allreg(struct i2c_client *client)
{
    s32 ret = -1, i = 0;
    u8 buf[50] = {0x8046 >> 8, 0x8046 & 0xff};
...
/*this func just test when no reg addr, what will it read from the chip*/

s32 gtp_i2c_read_with_no_regaddr(struct i2c_client *client)
{
    struct i2c_msg msgs;
    u8 buf[30]={0};
    s32 ret=-1, i = 0;
    ...
    msgs.len   = 30;
    msgs.buf   = &buf[0];            ///buf[0] buf[1] is reg addr
    msgs.scl_rate = 100 * 1000;    // for Rockchip
    msgs.udelay = 5;
    ...
    if((retries >= 5))
    {
	printk("test read error\n");
    }

    for(i = 0; i < 30; i++)
    {
        printk("buf[%d] = 0x%x , ret = %d \n", i, buf[i], ret);
    }
    return ret;
}

s32 gtp_read_test(struct i2c_client *client)
{
    return gtp_i2c_read_with_no_regaddr(client);
}
...
static s8 gtp_request_input_dev(struct goodix_ts_data *ts)
{
...
#endif
    ts->abs_x_max = ts->nwd_ts_panel->pixel_x;
    ts->abs_y_max = ts->nwd_ts_panel->pixel_y;
    input_set_abs_params(ts->input_dev, ABS_MT_POSITION_X, 0, ts->nwd_ts_panel->pixel_x, 0, 0);
    input_set_abs_params(ts->input_dev, ABS_MT_POSITION_Y, 0, ts->nwd_ts_panel->pixel_y, 0, 0);

    printk("ts->abs_x_max : %d, ts->abs_y_max : %d", ts->abs_x_max, ts->abs_y_max);
    ...
    ts->input_dev->id.bustype = BUS_I2C; ts->input_dev->id.vendor = 0xDEAD;
...
static ssize_t is_cts_get(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct goodix_ts_data *ts = i2c_get_clientdata(client);

	return sprintf(buf, "%d %s\n", ts->nwd_ts_panel->index, "write reg to interface read all registers");

}


static ssize_t is_cts_set(struct device *dev,
        struct device_attribute *attr, const char *buf, size_t count)
{
	...
	struct i2c_client *client = to_i2c_client(dev);
	struct goodix_ts_data *ts = i2c_get_clientdata(client);
	struct input_absinfo *x_absinfo, *y_absinfo;
	...
	ret = strict_strtol(buf, 10, &index);
	if(0 == ret)	
	{
		for(i = 0; nwd_panel[i] != NULL; i++)	
		{
			if(nwd_panel[i]->index == index)
			{
				ts->nwd_ts_panel = nwd_panel[i];
				x_absinfo = &ts->input_dev->absinfo[ABS_MT_POSITION_X];
				y_absinfo = &ts->input_dev->absinfo[ABS_MT_POSITION_Y];

				x_absinfo->maximum = ts->nwd_ts_panel->pixel_x;	
				y_absinfo->maximum = ts->nwd_ts_panel->pixel_y;	
				...
				if(ret < 0)
				{
			    	...
			    	printk("index : %ld, ts : %s \n", index, nwd_panel[i]->name);	
				return count;
			}
		}
	}
    ...
    {
	ret = gtp_read_allreg(ts->client);
    }
	

    if(!strncmp(buf, "test", 4))
    {
	ret = gtp_read_test(ts->client);
    }

    if(!strncmp(buf, "printon", 7))
    {
	print_coordinate = 1;
    }

    if(!strncmp(buf, "printoff", 8))
    {
	print_coordinate = 0;
    }
...
static DEVICE_ATTR(cts, S_IWUGO | S_IRUGO,
        is_cts_get, is_cts_set);

static struct attribute *cts_sysfs_attrs[] = {
    &dev_attr_cts.attr,
    NULL
};

static const struct attribute_group cts_sysfs = {
    .attrs  = cts_sysfs_attrs,
};
/*******************************************************
Function:
    I2c probe.
 ...
 static int goodix_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    ret = sysfs_create_group(&client->dev.kobj, &cts_sysfs);
    if (ret)
        return ret;
#if 0
    proc_entry_set_ts = create_proc_entry("set_gt9xx", 0666, NULL);
    if (NULL == proc_entry_set_ts) {
        printk(KERN_INFO "%s:  Couldn't create proc entry\n", __FUNCTION__);
        return  -ENOMEM;
    }

    proc_entry_set_ts->write_proc  = gt9xx_write_proc;
#endif

#if GTP_HAVE_TOUCH_KEY

    init_timer(&gtp_key_timer);
    //gtp_key_timer.expires = jiffies + msecs_to_jiffies(30);
...
static int __devinit goodix_ts_init(void)
{
    ...
    GTP_DEBUG_FUNC();   
    init_panel = set_panel(project_id);
    if(init_panel == NULL)
    {
	init_panel = default_panel;
    	GTP_INFO("GTP driver use default config ...");
    }
    printk("\ntouchscreen: project_id : %d, paramater : %s \n", project_id, init_panel->name);
