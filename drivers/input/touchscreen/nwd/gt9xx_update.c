...
st_update_msg_hwtst update_msg_hwtst;
u16 show_len_hwtst;
u16 total_len_hwtst;
u8 got_file_flag_hwtst = 0;
u8 searching_file_hwtst = 0;
...
extern s32 gtp_i2c_read_dbl_check_hwtst(struct i2c_client *, u16, u8 *, int);
#if GTP_ESD_PROTECT
...
*********************************************************/
s32 gup_i2c_read_hwtst(struct i2c_client *client, u8 *buf, s32 len)
{
...
*********************************************************/
s32 gup_i2c_write_hwtst(struct i2c_client *client,u8 *buf,s32 len)
{
    struct i2c_msg msg;
    s32 ret=-1;
    s32 retries = 0;
...
static s32 gup_init_panel(struct goodix_ts_data *ts)
{
    ...
    {
        ret = gtp_i2c_read_dbl_check_hwtst(ts->client, GTP_REG_SENSOR_ID, &sensor_id, 1);
        if (SUCCESS == ret)
        {
            if (sensor_id >= 0x06)
            {
    ...
    ret = gtp_i2c_read_dbl_check_hwtst(ts->client, GTP_REG_CONFIG_DATA, &opr_buf[0], 1);
...
static u8 gup_get_ic_msg(struct i2c_client *client, u16 addr, u8* msg, s32 len)
{
    ...
    for (i = 0; i < 5; i++)
    {
        if (gup_i2c_read_hwtst(client, msg, GTP_ADDR_LENGTH + len) > 0)
        {
            break;
        }
    }
...
static u8 gup_set_ic_msg(struct i2c_client *client, u16 addr, u8 val)
{
    ...
    for (i = 0; i < 5; i++)
    {
        if (gup_i2c_write_hwtst(client, msg, GTP_ADDR_LENGTH + 1) > 0)
        {
            break;
        }
    }
...
static u8 gup_get_ic_fw_msg(struct i2c_client *client)
{
    ...
    ret = gtp_i2c_read_dbl_check_hwtst(client, GUP_REG_HW_INFO, &buf[GTP_ADDR_LENGTH], 4);
    if (FAIL == ret)
    {
    ...
    for(i=0; i<4; i++)
    {
        update_msg_hwtst.ic_fw_msg.hw_info[i] = buf[GTP_ADDR_LENGTH + 3 - i];
    } 
    GTP_DEBUG("IC Hardware info:%02x%02x%02x%02x", update_msg_hwtst.ic_fw_msg.hw_info[0], update_msg_hwtst.ic_fw_msg.hw_info[1],
                                                   update_msg_hwtst.ic_fw_msg.hw_info[2], update_msg_hwtst.ic_fw_msg.hw_info[3]);
    ...
    for(retry=0; retry<2; retry++)
    {
        ...
        update_msg_hwtst.force_update = buf[GTP_ADDR_LENGTH];
        if((0xBE != update_msg_hwtst.force_update)&&(!retry))
        {
    ...
    GTP_DEBUG("IC force update flag:0x%x", update_msg_hwtst.force_update);
    
    // step3:get pid & vid
    ret = gtp_i2c_read_dbl_check_hwtst(client, GUP_REG_PID_VID, &buf[GTP_ADDR_LENGTH], 6);
    if (FAIL == ret)
    {
    ...
    memset(update_msg_hwtst.ic_fw_msg.pid, 0, sizeof(update_msg_hwtst.ic_fw_msg.pid));
    memcpy(update_msg_hwtst.ic_fw_msg.pid, &buf[GTP_ADDR_LENGTH], 4);
    GTP_DEBUG("IC Product id:%s", update_msg_hwtst.ic_fw_msg.pid);
    ...
    /*|-----FLASH-----RAM-----|
      ...
      |------9110-----9110----|
      |------9110P----9111----|*/
    if(update_msg_hwtst.ic_fw_msg.pid[0] != 0)
    {
        if(!memcmp(update_msg_hwtst.ic_fw_msg.pid, "9111", 4))
        {
            GTP_DEBUG("IC Mapping Product id:%s", update_msg_hwtst.ic_fw_msg.pid);
            memcpy(update_msg_hwtst.ic_fw_msg.pid, "9110P", 5);
        }
    }
    
    update_msg_hwtst.ic_fw_msg.vid = buf[GTP_ADDR_LENGTH+4] + (buf[GTP_ADDR_LENGTH+5]<<8);
    GTP_DEBUG("IC version id:%04x", update_msg_hwtst.ic_fw_msg.vid);
    
    return SUCCESS;
}

s32 gup_enter_update_mode_hwtst_hwtst_hwtst_hwtst(struct i2c_client *client)
{
    s32 ret = -1;
    s32 retry = 0;
    u8 rd_buf[3];
...
void gup_leave_update_mode_hwtst(void)
{
...
static u8 gup_enter_update_judge(st_fw_head *fw_head)
{
    ...
    GTP_DEBUG("IC HARDWARE INFO:%02x%02x%02x%02x", update_msg_hwtst.ic_fw_msg.hw_info[0], update_msg_hwtst.ic_fw_msg.hw_info[1],
                                                   update_msg_hwtst.ic_fw_msg.hw_info[2], update_msg_hwtst.ic_fw_msg.hw_info[3]);
    GTP_DEBUG("IC PID:%s", update_msg_hwtst.ic_fw_msg.pid);
    GTP_DEBUG("IC VID:%04x", update_msg_hwtst.ic_fw_msg.vid);

    //First two conditions
    if ( !memcmp(fw_head->hw_info, update_msg_hwtst.ic_fw_msg.hw_info, sizeof(update_msg_hwtst.ic_fw_msg.hw_info)))
    {
        GTP_DEBUG("Get the same hardware info.");
        if( update_msg_hwtst.force_update != 0xBE )
        {
        ...
        // 20130523 start
        if (strlen(update_msg_hwtst.ic_fw_msg.pid) < 3)
        {
        ...
        else
        {
            for (i = 0; i < 3; i++)
            {
                if ((update_msg_hwtst.ic_fw_msg.pid[i] < 0x30) || (update_msg_hwtst.ic_fw_msg.pid[i] > 0x39))
                {
        ...
        if (( !memcmp(fw_head->pid, update_msg_hwtst.ic_fw_msg.pid, (strlen(fw_head->pid)<3?3:strlen(fw_head->pid))))||
            (!memcmp(update_msg_hwtst.ic_fw_msg.pid, "91XX", 4))||
            (!memcmp(fw_head->pid, "91XX", 4)))
        {
            ...
            //The third condition
            if (fw_head->vid > update_msg_hwtst.ic_fw_msg.vid)
            {
...
static s8 gup_update_config(struct i2c_client *client)
{
    ...
    if(NULL == update_msg_hwtst.cfg_file)
    {
        GTP_ERROR("[update_cfg]No need to upgrade config!");
        return FAIL;
    }
    file_len = update_msg_hwtst.cfg_file->f_op->llseek(update_msg_hwtst.cfg_file, 0, SEEK_END);
    ...
    update_msg_hwtst.cfg_file->f_op->llseek(update_msg_hwtst.cfg_file, 0, SEEK_SET);
    
    GTP_DEBUG("[update_cfg]Read config from file.");
    ret = update_msg_hwtst.cfg_file->f_op->read(update_msg_hwtst.cfg_file, (char*)pre_buf, file_len, &update_msg_hwtst.cfg_file->f_pos);
    if(ret<0)
    {
...
//    file_config[chip_cfg_len+1] = 0x01;
    ...
    while(i++ < 5)
    {
        ret = gup_i2c_write_hwtst(client, file_config, file_cfg_len);
        if(ret > 0)
        {
...
static u8 gup_check_update_file(struct i2c_client *client, st_fw_head* fw_head, u8* path)
{
    ...
    if (path)
    {
        ...
        update_msg_hwtst.file = filp_open(path, O_RDONLY, 0);

        if (IS_ERR(update_msg_hwtst.file))
        {
    ...
    else
    {
        ...
        // update config
        update_msg_hwtst.cfg_file = filp_open(CONFIG_FILE_PATH_1, O_RDONLY, 0);
        if (IS_ERR(update_msg_hwtst.cfg_file))
        {
        ...
        else
        {
            ...
            filp_close(update_msg_hwtst.cfg_file, NULL);
        }
        ...
        update_msg_hwtst.file = filp_open(UPDATE_FILE_PATH_2, O_CREAT | O_RDWR, 0666);
        if ((IS_ERR(update_msg_hwtst.file)))
        {
            GTP_ERROR("Failed to Create file: %s for fw_header!", UPDATE_FILE_PATH_2);
            return FAIL;
        }
        update_msg_hwtst.file->f_op->llseek(update_msg_hwtst.file, 0, SEEK_SET);
        update_msg_hwtst.file->f_op->write(update_msg_hwtst.file, (char *)header_fw_array, sizeof(header_fw_array), &update_msg_hwtst.file->f_pos);
        filp_close(update_msg_hwtst.file, NULL);
        update_msg_hwtst.file = filp_open(UPDATE_FILE_PATH_2, O_RDONLY, 0);
#else
        ...
        searching_file_hwtst = 1;
        for (i = 0; i < GUP_SEARCH_FILE_TIMES; i++)
        {
            if (searching_file_hwtst == 0)
            {
            ...
            if(!(got_file_flag_hwtst&0x0F))
            {
                update_msg_hwtst.file = filp_open(search_update_path, O_RDONLY, 0);
                if(!IS_ERR(update_msg_hwtst.file))
                {
                    GTP_DEBUG("Find the bin file");
                    got_file_flag_hwtst |= 0x0F;
                }
            }
            if(!(got_file_flag_hwtst&0xF0))
            {
                update_msg_hwtst.cfg_file = filp_open(search_cfg_path, O_RDONLY, 0);
                if(!IS_ERR(update_msg_hwtst.cfg_file))
                {
                    GTP_DEBUG("Find the cfg file");
                    got_file_flag_hwtst |= 0xF0;
                }
            }
            
            if(got_file_flag_hwtst)
            {
                if(got_file_flag_hwtst == 0xFF)
                {
                    break;
                }
                else
            ...
            GTP_DEBUG("%3d:Searching %s %s file...", i, (got_file_flag_hwtst&0x0F)?"":"bin", (got_file_flag_hwtst&0xF0)?"":"cfg");
            msleep(3000);
        }
        searching_file_hwtst = 0;
        kfree(search_update_path);
        kfree(search_cfg_path);
    ...
    update_msg_hwtst.old_fs = get_fs();
    set_fs(KERNEL_DS);

    update_msg_hwtst.file->f_op->llseek(update_msg_hwtst.file, 0, SEEK_SET);
    //update_msg_hwtst.file->f_pos = 0;

    ret = update_msg_hwtst.file->f_op->read(update_msg_hwtst.file, (char*)buf, FW_HEAD_LENGTH, &update_msg_hwtst.file->f_pos);
    if (ret < 0)
    {
    ...
    {
        u16 temp;
        ret = update_msg_hwtst.file->f_op->read(update_msg_hwtst.file, (char*)buf, 2, &update_msg_hwtst.file->f_pos);
        if (ret < 0)
        {
...
load_failed:
    set_fs(update_msg_hwtst.old_fs);
    return FAIL;
}
...
static u8 gup_burn_proc(struct i2c_client *client, u8 *burn_buf, u16 start_addr, u16 total_len_hwtstgth)
{
    s32 ret = 0;
    ...
    GTP_DEBUG("Begin burn %dk data to addr 0x%x", (total_len_hwtstgth/1024), start_addr);
    while(burn_length < total_len_hwtstgth)
    {
        GTP_DEBUG("B/T:%04d/%04d", burn_length, total_len_hwtstgth);
        frame_length = ((total_len_hwtstgth - burn_length) > PACK_SIZE) ? PACK_SIZE : (total_len_hwtstgth - burn_length);
        ...
        {
            ret = gup_i2c_write_hwtst(client, wr_buf, GTP_ADDR_LENGTH + frame_length);
            if(ret <= 0)
            {
                GTP_ERROR("Write frame data i2c error.");
                continue;
            }
            ret = gup_i2c_read_hwtst(client, rd_buf, GTP_ADDR_LENGTH + frame_length);
            if(ret <= 0)
            {
...
static u8 gup_load_section_file(u8* buf, u16 offset, u16 length)
{
    ...
    if(update_msg_hwtst.file == NULL)
    {
        GTP_ERROR("cannot find update file,load section file fail.");
        return FAIL;
    }
    update_msg_hwtst.file->f_pos = FW_HEAD_LENGTH + offset;
    
    ret = update_msg_hwtst.file->f_op->read(update_msg_hwtst.file, (char*)buf, length, &update_msg_hwtst.file->f_pos);
    if(ret < 0)
    {
...
s32 gup_update_proc_hwtst(void *dir)
{
    s32 ret = 0;
    u8  retry = 0;
    st_fw_head fw_head;
    ...
    if (searching_file_hwtst)
    {
        searching_file_hwtst = 0;     // exit .bin update file searching 
        GTP_INFO("Exiting searching .bin update file...");
        while ((show_len_hwtst != 200) && (show_len_hwtst != 100))     // wait for auto update quitted completely
        {
            msleep(100);
        }
    }
    
    update_msg_hwtst.file = NULL;
...
#endif
    ret = gup_enter_update_mode_hwtst_hwtst_hwtst_hwtst(i2c_connect_client);
    if(FAIL == ret)
    {
...
file_fail:
    if(update_msg_hwtst.file && !IS_ERR(update_msg_hwtst.file))
    {
        filp_close(update_msg_hwtst.file, NULL);
    }
    show_len_hwtst = 200;
...
u8 gup_init_update_proc(struct goodix_ts_data *ts)
{
    ...
    thread = kthread_run(gup_update_proc_hwtst, (void*)NULL, "guitar_update");
    if (IS_ERR(thread))
    {
