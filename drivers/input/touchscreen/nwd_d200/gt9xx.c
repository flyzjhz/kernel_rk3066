...
if (jiffies_to_msecs (jiffies - time_prev5) >260)
{
	...
	///input_report_key( input_dev_g, KEY_F1,1);
	//////task_key_to1();
	press_once = 0;
	///short_key_to1();
		
 	input_sync(input_dev_g); key_pressed=5;short_pressed = 1;
	///input_report_key(input_dev_g, KEY_F1,0);input_sync(input_dev_g);
	///task_key_to0();
...
if(press_once==40) 
		{
			...
			key_released = 0;
			press_once = 0;
input_report_key(input_dev_g, 252,0);input_sync(input_dev_g);
 ...
 ///	input_sync(ts->input_dev);
	...
	{	/// printk("===========%d %s====\n",__LINE__,__FUNCTION__);
		...
		if(is_long_pressed == 1){
			...
			input_report_key(ts->input_dev, KEY_VOLUMEDOWN_NWD, 1); ///nwd_dbg_tp();
		}		
		key_pressed= 3;
	}
	if( 235< x && x <330 )
...
static void gtp_touch_up(struct goodix_ts_data* ts, s32 id)
{
	...
	if(key_pressed==2){
		...
		home_key_to0();	input_sync(ts->input_dev);key_pressed= 9;}
		return;
	}
	if(key_pressed==4){
		if(is_long_pressed == 0){
