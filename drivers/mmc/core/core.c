...
/* Order's important: probe SDIO, then SD, then MMC */
	...
	{
			//	sdio_reset(host);
			printk("sec hostname = %s\n",mmc_hostname(host));
			if (host->rk_sdio_used == 1 && boot_nr !=0 )
			{
					printk("sdio??\n");
					sdio_reset(host);
					if (!(init_ret=mmc_attach_sdio(host)))
...
unsigned char wifi_on_flag = 0;
EXPORT_SYMBOL(wifi_on_flag);
...
//lusterzhang 	
	...
	printk("===%d %s= %d=%d=\n",__LINE__,__FUNCTION__,wifi_on_flag, host->host_dev_id);
/*	
	if((wifi_on_flag==0) && (memcmp(mmc_hostname(host),"mmc1",4)==0))
	{	
	      printk("\n\n\n\n\n\n\n\n\n===%d %s===\n",__LINE__,__FUNCTION__);
	      return;	
	}
*/
	if (host->rescan_disable) {	printk("===%d %s===\n",__LINE__,__FUNCTION__);
		return;
	}
