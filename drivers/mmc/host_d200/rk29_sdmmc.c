...
static int rk29_sdmmc_get_cd(struct mmc_host *mmc)
{
    ...
    for(i=0;i<5;i++)
        ...
        case 1:
 {
            //#if defined(CONFIG_MACH_RK30_PS2_VEHICLE) || defined(CONFIG_MACH_RKPX2_VEHICLE_SDK)//RK29SDK_MMC1_CARD_DETECT_N
            //    if(host->det_pin.io == INVALID_GPIO)
        	//	    return 1;
        	 //   cdetect = rk29_sdmmc_read(host->regs, SDMMC_CDETECT);
  //   cdetect = (cdetect & SDMMC_CARD_DETECT_N)?0:1;
                ...
                #if 1 ///defined(CONFIG_SDMMC1_RK29_SDCARD_DET_FROM_GPIO)
			int i=0, cdetect1 , cdetect2 ;
                    if(host->det_pin.io == INVALID_GPIO)
                    ;///	return 1;
   for(i=0;i<5;i++)
                    {
                        udelay(1000);
                        cdetect1 = gpio_get_value(RK30_PIN3_PC6);  
                        udelay(1000);
                        cdetect2 = gpio_get_value(RK30_PIN3_PC6); 
      printk("+++++%d+++++++++%d+++++++cdetect1%d++++++cdetect1%d+++++++++++++\n",__LINE__,gpio_get_value(RK30_PIN3_PC6),cdetect1,cdetect2);
                        if(cdetect1 == cdetect2)
                            break;
                    }
 cdetect = cdetect2;          
                    if(host->det_pin.enable)
                        ...
                        cdetect = cdetect?0:1;
 			 ///cdetect = cdetect?1:0;
     ///printk("+++%d+++++++++%d+++++++cdetect1%d++++++++++++++++\n",__LINE__,gpio_get_value(RK30_PIN3_PC6),cdetect);
                #else
   if(host->det_pin.io == INVALID_GPIO)
        		    return 1;
            ...
            #endif 
               set_bit(RK29_SDMMC_CARD_PRESENT, &host->flags);  //add by lusterzhang
               return cdetect;   //add by lusterzhang
        }
        case 2:
        default:
 set_bit(RK29_SDMMC_CARD_PRESENT, &host->flags);
...
#if defined(CONFIG_USE_SDMMC0_FOR_WIFI_DEVELOP_BOARD)///||defined(CONFIG_USE_SDMMC1_FOR_WIFI_DEVELOP_BOARD)//Temporary add mmc1 for vehicle-project
//     	printk("++++++++++%d+++++++++%d+++++++cdetect1%d++++++++++++++++\n",__LINE__,gpio_get_value(RK30_PIN3_PC6),cdetect);
	set_bit(RK29_SDMMC_CARD_PRESENT, &host->flags);
    return 1;
#endif
/// printk("++++%d+++++++++%d+++++++cdetect%d++++++++++++++++\n",__LINE__,gpio_get_value(RK30_PIN3_PC6),cdetect);
	 return cdetect;
}
...
static int rk29_sdmmc_start_request(struct mmc_host *mmc )
{
    ...
    {        
	    ...
	    //add by lusterzhang 
            if(memcmp(mmc_hostname(host->mmc),"mmc1",4)==0)
            {
                printk("rk29_sdmmc_start_request mmc1..........\n");
                return ret;
            }
...
int sd_in_boot_eject;
 bool present_old = 0;
unsigned long present_cnt=1;
extern suspend_card_nwd;
static irqreturn_t rk29_sdmmc_interrupt(int irq, void *dev_id)
{
	struct rk29_sdmmc	*host = dev_id;
	u32			status,  pending;
	bool present;
///	static bool present_old = 0;
         if(present_cnt==1)
		present_old = 0; present_cnt=3; ///prsent_cnt_only for static fun
	int sdio_irq=0;
    ...
    {
	printk("LINE：%d SDMMC_INT_CD.........dev_id%d\n",__LINE__,host->host_dev_id);
	...
	printk("caonidie  SDMMC_INT_CD present_old=%u,present=%u.........\n",present_old,present);
 if((present != present_old) ||(present ==0 && present_old==0 &&sd_in_boot_eject==4)
|| (suspend_card_nwd==1&&present==1&&present_old==1&&host->host_dev_id==1)) ///for mod susoned-wifi,/// by lusterzhang
    	{    	 sd_in_boot_eject = 9; if(host->host_dev_id==1) suspend_card_nwd=9;
 		printk("LINE：%d the may skip func .........\n",__LINE__);
 //   	    rk28_send_wakeup_key(); //wake up backlight
    	    ...
    	    if(present)
    	    {
        	    ...
        	    {
			printk("SDMMC_INT_CD 11111.........\n");
        	        mod_timer(&host->detect_timer, jiffies + msecs_to_jiffies(RK29_SDMMC_REMOVAL_DELAY));//del by lusterzhang//RK29_SDMMC_REMOVAL_DELAY));
        	    }
        	    else
        	    {
			printk("SDMMC_INT_CD 22222.........\n");
        	        mod_timer(&host->detect_timer, jiffies + msecs_to_jiffies(RK29_SDMMC_REMOVAL_DELAY*2));
        	    }
    	    }
    	    else
    	    {
    	        ...
    	        host->mmc->re_initialized_flags = 0;
		printk("SDMMC_INT_CD 3333333.........\n");
    	        mmc_detect_change(host->mmc, 2);//modify by lusterzhang 200
    	    }

    	}
         present_old = present;///wj 
        goto Exit_INT;
...
static void rk29_sdmmc_detect_change_work(struct work_struct *work)
{
 ...
 //   rk28_send_wakeup_key();  //del by lusterzhang
...
static int rk29_sdmmc_probe(struct platform_device *pdev)
{
	...
	host->mmc->rk_sdio_used = pdata->rk_sdio_used; 
	//detect pin info
...
///wj
	...
	{  //add by lusterzhang
		clk_set_rate(host->clk,20000000);
		clk_enable(host->clk);	
	}
...
module_exit(rk29_sdmmc_emmc_exit);
#endif
