...
u8 config[GTP_CONFIG_MAX_LENGTH + GTP_ADDR_LENGTH]
                = {GTP_REG_CONFIG_DATA >> 8, GTP_REG_CONFIG_DATA & 0xff}; ///the 2 is addr ,following is value
...
*/
#define GTP_GPIO_OUTPUT(x,y) gpio_direction_output(x,0) ///(, 0);   // begin select I2C slave addr
#define GTP_GPIO_AS_INT(x) gpio_direction_input(x)
#define GTP_GPIO_AS_INPUT(x) gpio_direction_input(x)
#define GTP_GPIO_REQUEST(x,y) gpio_request(x,y)  ///gpio,label
#define GTP_GPIO_FREE(x)        gpio_free(x)
...
s32 gtp_i2c_read(struct i2c_client *client, u8 *buf, s32 len)
{
    ...
    msgs[0].buf   = &buf[0];   ///0 1 is the addr to wr 
