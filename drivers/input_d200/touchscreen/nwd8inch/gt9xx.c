...
s32 gtp_i2c_write(struct i2c_client *client,u8 *buf,s32 len)
{
    ...
    #endif
        GTP_DEBUG("I2C wr-communication timeout, resetting chip...");
        gtp_reset_guitar(client, 10);
    }
    return ret;
}
...
/***                   sdk
		input_mt_slot(ts->input_dev, i);
			...
			input_report_abs(ts->input_dev, ABS_MT_POSITION_Y, info_buf[i].y);
                        DBG("touch point %d %d >>x:%d>>y:%d\n",i,info_buf[i].status,info_buf[i].x,info_bu
*/
...
///input_mt_report_slot_state(ts->input_dev, MT_TOOL_FINGER, false); ///?need to add ?,rk drv have  
    ...
    input_report_abs(ts->input_dev, ABS_MT_POSITION_X,((1024-x)*1024)/940);
    input_report_abs(ts->input_dev, ABS_MT_POSITION_Y, 600-y);
#else
    input_report_abs(ts->input_dev, ABS_MT_POSITION_X, x); ///wj
    input_report_abs(ts->input_dev, ABS_MT_POSITION_Y, (y*5)/4);
   ...
   /// GTP_DEBUG("ID:%d, X:%d, Y:%d, W:%d", id, x, y, w);
	///printk("===nwd8INCH ID:%d, X:%d, Y:%d, W:%d\n", id,((1024-x)*1024)/940, 600-y, w);
}
...
*/
    ...
    ///input_mt_report_slot_state(ts->input_dev, MT_TOOL_FINGER, false);///wj add
    ...
    if (pre_touch || touch_num)
    {
        ...
        if(touch_num)
        {
            ...
            if ((id == 128))  
            {
                ...
                input_report_abs(ts->input_dev, ABS_MT_POSITION_X,  1020-input_x);
                input_report_abs(ts->input_dev, ABS_MT_POSITION_Y, 600-input_y);
...
static s32 gtp_init_panel(struct goodix_ts_data *ts)   ///wr cfg
{
    ...
    }     ///nwd();nwd();return -1; ///have int 
    ret = gtp_send_cfg(ts->client);
    if (ret < 0)
    {
        GTP_ERROR("Send config error.");
    }
	////nwd();nwd();return -1; ///no int
...
static int goodix_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    GTP_DEBUG_FUNC();
    printk("+++/sizeof(bmp_logo_rle):%d+++++++++\n",sizeof(bmp_logo_rle)/sizeof(bmp_logo_rle[0])  );
...
#endif
   /// nwd_dbg_tp();nwd_dbg_tp();nwd();nwd(); return -1;   ///h
	...
	gtp_int_sync(50);///wj add for no int
    ret = gtp_request_input_dev(ts);
    if (ret < 0)
    {
        GTP_ERROR("GTP request input dev failed");
    }
    	///nwd_dbg_tp();nwd();nwd(); return -1;  
    ...
    nwd_dbg_tp();///nwd();return -1;///h
#if GTP_CREATE_WR_NODE
    init_wr_node(client);nwd();nwd();
#endif
