...
static void tpd_down(int id, int x, int y, int size)
{
    ...
    input_report_abs(ts_g->input_dev, ABS_PRESSURE, 128);
    input_report_key(ts_g->input_dev, BTN_TOUCH, 1);
    input_report_abs(ts_g->input_dev, ABS_MT_TOUCH_MAJOR, 128);
    input_report_abs(ts_g->input_dev, ABS_MT_WIDTH_MAJOR, 128);
    input_report_abs(ts_g->input_dev, ABS_MT_POSITION_X, x);
    input_report_abs(ts_g->input_dev, ABS_MT_POSITION_Y, y);
    input_report_abs(ts_g->input_dev, ABS_MT_TRACKING_ID, id);
    input_mt_sync(ts_g->input_dev);
  	printk("[mtk-tpd] tpd_down : id=%d x=%d y=%d\n", id, x, y);
    ///TPD_EM_PRINT(x, y, x, y, size, 1);
#else
    input_mt_slot(ts_g->input_dev, id); 
///input_mt_report_slot_state(ts->input_dev, MT_TOOL_FINGER, false); ///?need to add ?,rk drv have  
    input_report_abs(ts_g->input_dev, ABS_MT_TRACKING_ID, id);
    input_report_abs(ts_g->input_dev, ABS_MT_POSITION_X,(x));
    input_report_abs(ts_g->input_dev, ABS_MT_POSITION_Y, y);
    input_report_abs(ts_g->input_dev, ABS_MT_TOUCH_MAJOR, 128); ///.press
    input_report_abs(ts_g->input_dev, ABS_MT_WIDTH_MAJOR, 128);
#endif 
	printk("[mtk-tpd] tpd_down-nwd %d : id=%d x=%d y=%d\n", id, x, y,__LINE__);
}
static void tpd_up(int id, int x, int y)
{
#if  !GTP_ICS_SLOT_REPORT 
    input_report_abs(ts_g->input_dev, ABS_PRESSURE, 0);
    input_report_key(ts_g->input_dev, BTN_TOUCH, 0);
    input_report_abs(ts_g->input_dev, ABS_MT_TOUCH_MAJOR, 0);
    input_report_abs(ts_g->input_dev, ABS_MT_WIDTH_MAJOR, 0);
    input_report_abs(ts_g->input_dev, ABS_MT_POSITION_X, x);
    input_report_abs(ts_g->input_dev, ABS_MT_POSITION_Y, y);
    input_report_abs(ts_g->input_dev, ABS_MT_TRACKING_ID, id);
    input_mt_sync(ts_g->input_dev);
#else
    input_mt_slot(ts_g->input_dev, id); ///	input_mt_report_slot_state(ts->input_dev, MT_TOOL_FINGER, false);  ?
    input_report_abs(ts_g->input_dev, ABS_MT_TRACKING_ID, -1);
    GTP_DEBUG("Touch id[%2d] release!", id);
   /// TPD_EM_PRINT(x, y, x, y, 0, 0);
    //TPD_DEBUG_PRINT_POINT( x, y, 0 );
   printk("[mtk-tpd] tpd_upjj1   : id=%d x=%d y=%d\n", id, x, y);
#endif 
	///   printk("[mtk-tpd] tpd_upjj2   : id=%d x=%d y=%d\n", id, x, y);
}
static int tpd_halt=0;
static int tpd_flag = 0;
s32 gtp_i2c_read(struct i2c_client *client, u8 *buf, s32 len) ;
s32 gtp_i2c_write(struct i2c_client *client,u8 *buf,s32 len);
#define i2c_read_bytes gtp_i2c_read
#define i2c_write_bytes gtp_i2c_write
static int touch_event_handler()
{
   /// struct sched_param param = { .sched_priority = RTPM_PRIO_TPD }; 
    int x, y, id, size, finger_num = 0;
    static u8 buffer[32], wrbuf[1] = { 0 };
    static u8 id_mask = 0;
    u8 cur_mask;
    int idx;
 ...
 ///       set_current_state(TASK_RUNNING); 
	...
	i2c_write_bytes(i2c_client, wrbuf, 1);
        ...
        i2c_read_bytes( i2c_client, buffer, 16);
                
        finger_num = 1;///5;/////buffer[0] & 0x07;
   ...
   for (idx = 0; idx < TPD_MAX_KEYS; idx++) {
	    ...
	    if (tpd != NULL && ts_g->input_dev != NULL)
...
cur_mask = 0;
	///for (idx=0; idx<finger_num; idx++) {
	    u8 *ptr = &buffer[idx*TPD_POINT_INFO_LEN];///&buffer[idx*TPD_POINT_INFO_LEN+2];
	   /// id = ptr[3]&0xf0>>4;///ptr[4]&0x07;
	    id = ptr[3]&0x0f;///ptr[4]&0x07;
	    id = 0;
	 ///   if (id < TPD_MAX_POINTS) {
		x = ptr[0]&0xff;
		x = x<< 4;
		x = x + ((ptr[2])>>4)&0x0f;
		y = ptr[1] + (((u8)ptr[2]) &0x0f);
		size = 28;

			x = ((u8)ptr[0]<<4) + (((u8)ptr[2]) &0x0f);
	///	printk("==========11111111111 x%d  ==============\n",x);

			y = ((u8)ptr[1]<<4) + ((((u8)ptr[2]) &0xf0)>>4 & 0x0f);
	////	printk("==========3333333333333 y%d  ==============\n",y);

/*
		x = ((u8)ptr[0]<<4)+ (((u8)ptr[2]) &0xf0)>>4;
		printk("==========2222222222222 x%d   ==============\n",
		x );


		x = ptr[0]<<8 + (((u8)ptr[2]) &0xf0)>>4;
		printk("==========2222222222222 x%d   ==============\n",
		x );
...
*/
		...
		y_history[id] = y;
   		////} else
		printk("22Invalid id %d  id:%d  x%d y%d  status:%d\n", id,idx*TPD_POINT_INFO_LEN,x,y ,ptr[3]&0x0f);
	/// }
	printk("==========2222222222222 x%s   %d==%d==%d==========\n",__FUNCTION__,__LINE__ ,cur_mask,id_mask);
	if (cur_mask != id_mask) {
	///if (1) {
	    u8 diff = cur_mask^id_mask;
	    idx = 0;
		printk("==========2222222222222 x%s   %d==============\n",__FUNCTION__,__LINE__ ,cur_mask,id_mask);
	    while (diff) {
...
*/
    msgs[0].flags = I2C_M_RD;
    msgs[0].addr  = client->addr;
    msgs[0].len   = len;/// - GTP_ADDR_LENGTH;
    msgs[0].buf   = &buf[0];///&buf[GTP_ADDR_LENGTH];
    msgs[0].scl_rate = 100 * 1000;
    msgs[0].udelay = 5;

    while(retries < 5)
    {
         ret = i2c_transfer(client->adapter, msgs, 1);/// ret = i2c_transfer(client->adapter, msgs, 2);
        if(ret == 1)break;
        retries++;
    }
    if((retries >= 5))
...
s32 gtp_i2c_read2(struct i2c_client *client, u8 *buf, s32 len)
{
  ...
  if((retries >= 5))
    ...
    #endif
        GTP_DEBUG("I2C rd-communication timeout, resetting chip...");
        gtp_reset_guitar(client, 10);
    }
    return ret;
}
...
static void goodix_ts_work_func(struct work_struct *work)
{
	...
	touch_event_handler();   gtp_irq_enable(ts_g);
	return ;
...
static s8 gtp_request_io_port(struct goodix_ts_data *ts)
{
  ...
  /***  gpio_set_value(GTP_RST_PORT,1);mdelay(300);
    gpio_set_value(GTP_RST_PORT,0);mdelay(300);
    gpio_set_value(GTP_RST_PORT,1);mdelay(300);*/
    gpio_set_value(GTP_RST_PORT,0);///mdelay(300);
...
static int goodix_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    GTP_DEBUG_FUNC();
    i2c_client = client;
    unsigned char tpd_fw_crc[2] = { 0 }, wrbuf[1] = { TPD_FW_CRC_REG };int err;
   unsigned char tpd_fw_crc2[3] = {TPD_FW_CRC_REG,0, 0 };
    //do NOT remove these logs
    printk("\n\n\n\n\n\n\n\n\n\n\n=============GTP Driver Version: %s", GTP_DRIVER_VERSION);
    printk("GTP Driver Built@%s, %s", __TIME__, __DATE__);
    printk("GTP I2C Address: 0x%02x", client->addr);

    i2c_connect_client = client;
	nwd_dbg_tp();
 	///gpio_set_value(GTP_RST_PORT,1);mdelay(3000);
	...
	nwd_dbg_tp();
    ts_g = kzalloc(sizeof(*ts_g), GFP_KERNEL);
	nwd_dbg_tp();
ts = ts_g;
///    ts = kzalloc(sizeof(*ts), GFP_KERNEL);
   ...
   pdata = client->dev.platform_data; ///
	nwd_dbg_tp();
    memset(ts_g, 0, sizeof(*ts_g));
	///ts_g= ts;
	nwd_dbg_tp();
    INIT_WORK(&ts_g->work, goodix_ts_work_func);
    ts_g->client = client;
    spin_lock_init(&ts_g->irq_lock);          // 2.6.39 later
    // ts->irq_lock = SPIN_LOCK_UNLOCKED;   // 2.6.39 & before
    i2c_set_clientdata(client, ts_g);
    
    ts_g->gtp_rawdiff_mode = 0;
    ...
    if (ret < 0)
    {
		...
		nwd_dbg_tp();
        gtp_reset_guitar(ts->client, 50); ///wj add for ct360
///while(1)
{
 ...
 ///i2c_read_bytes( client, wrbuf, 1);
  printk( " FW CRC :  %x %x\n", tpd_fw_crc[0], tpd_fw_crc[1] );
    ...
    err = gtp_i2c_read2( client, tpd_fw_crc2, 3);
    if (err) {
 	printk( " FW CRC2 :  %x %x\n", tpd_fw_crc[0], tpd_fw_crc[1] );
       printk( " Fail to read FW CRC info %d\n", err );
    } else {
  ...
  /*** */ ret = gtp_init_panel(ts);
	...
	nwd_dbg_tp();
    ret = gtp_request_input_dev(ts_g);
    if (ret < 0)
    {
        GTP_ERROR("GTP request input dev failed");
    }
    	nwd_dbg_tp();
    ret = gtp_request_irq(ts_g); 
    if (ret < 0)
    {	nwd_dbg_tp();
