...
#if GTP_DEBUG_ON
    static const int  key_codes[] = {KEY_HOME, KEY_BACK, KEY_F1, KEY_SEARCH};
    static const char *key_names[] = {"Key_Home", "Key_Back", "KEY_F1", "Key_Search"};
#endif
...
nwd_dbg_tp();
    ...
    if (ts->fixed_cfg)
    {   printk("Driver send config.==============================1=========\n");
        GTP_INFO("Ic fixed config, no config sent!");
        return 0;
    }
    else if (ts->pnl_init_error)
    {   printk("Driver send config.==============================2=========\n");
        GTP_INFO("Error occured in init_panel, no config sent");
        return 0;
    }
    
    printk("Driver send config.==============================3=========\n");
    for (retry = 0; retry < 5; retry++)
    {	   printk("Driver send config.==============================4=========\n");
        ret = gtp_i2c_write(client, config , GTP_CONFIG_MAX_LENGTH + GTP_ADDR_LENGTH);
        if (ret > 0)
        {   printk("Driver send config.==============================5=========\n");
            break;
        }
    }
#endif
...
///extern is_long_pressed; ///hbs rse old-board.
static  long is_long_pressed = 0;
void report_mute_key(unsigned char on);

int machine_type=9999;
...
{
////printk("===========%d %s==long_pressed%d==\n",__LINE__,__FUNCTION__,machine_type);
      if(machine_type==688){
printk("===========%d %s==long_pressed%d==\n",__LINE__,__FUNCTION__,machine_type);
...
///printk("===========%d %s==long_pressed%d==\n",__LINE__,__FUNCTION__,machine_type);
      if(machine_type==712){
///printk("===========%d %s==long_pressed%d==\n",__LINE__,__FUNCTION__,machine_type);
	input_report_key( input_dev_g, 251,1);
	//////task_key_to1();
	press_once = 0;
	///short_key_to1();
		
 	input_sync(input_dev_g); key_pressed=5;short_pressed = 1;
	input_report_key(input_dev_g, 251,0);input_sync(input_dev_g);
	}
...
unsigned char is_menu_long_pressed = 0;
/*******************************************************
Function:
...
Output:
    None.
*********************************************************/
unsigned long time_prev_wake = 0;
static void gtp_touch_down(struct goodix_ts_data* ts,s32 id,s32 x,s32 y,s32 w)
{
#if GTP_CHANGE_X2Y
    GTP_SWAP(x, y);
#endif
    if(y>824) ///GTP_HAVE_TOUCH_KEY
	goto key_event;
	if(jiffies_to_msecs(jiffies - time_prev_wake) < 1200)  return ;
	if(is_long_pressed == 1){         
					report_mute_key(1);
					report_mute_key(0);is_long_pressed = 0;	time_prev_wake = jiffies;
					return ;
				}
  ...
  ///  input_report_abs(ts->input_dev, ABS_MT_POSITION_X, y*1280/1000);
  ///  input_report_abs(ts->input_dev, ABS_MT_POSITION_Y, x*4/3);
if(sensor_id_g==2){
    input_report_abs(ts->input_dev, ABS_MT_POSITION_X, x);
    input_report_abs(ts->input_dev, ABS_MT_POSITION_Y, y);}
else if(sensor_id_g==0){ ///jianchuan 6.5 
    ///printk("ID:%da, X:%d, Y:%d, W:%d %s \n", id, x, 600-y, w,__FILE__);
...
key_event:  
  if( sensor_id_g==0 && x>1024) ///GTP_HAVE_TOUCH_KEY
    {  ///    #define GTP_KEY_TAB  {KEY_F1,  KEY_BACK,KEY_HOME,KEY_VOLUMEDOWN_NWD,KEY_VOLUMEUP_NWD,KEY_DISPLAYTOGGLE_NWD}
	int key_value = 1; static int timecnt=1; 
           machine_type=688;	
	/// printk("===========%d %s====\n",__LINE__,__FUNCTION__);
	if( 315< y && y <400 ) ///back
	{   	if (timecnt != 1  && jiffies_to_msecs (jiffies - time_prev) < 400 ) return ;    
		/// printk("===========%d %s====\n",__LINE__,__FUNCTION__);
				if(is_long_pressed == 1){
					report_mute_key(1);
					report_mute_key(0);is_long_pressed = 0;	
				}else{				
    ///input_report_key(ts->input_dev, KEY_BACK, 1);	input_sync(ts->input_dev);
     input_report_key(ts->input_dev, 253, 1);	input_sync(ts->input_dev);
				}
				///input_report_key(ts->input_dev, KEY_BACK, 0); 	input_sync(ts->input_dev);
				///nwd_dbg_tp();
				key_pressed= 1;
	}

	if( 410< y && y <530 ) ///6.5INCH,PWR
{

if (timecnt != 1  && jiffies_to_msecs (jiffies - time_prev) < 200 ) return ;///printk("===========%d %s====\n",__LINE__,__FUNCTION__);
                if(is_long_pressed == 1){
                                        report_mute_key(1);
                                        report_mute_key(0);is_long_pressed = 0;
                }else{
                        /***     previous is only short press
                        input_report_key(ts->input_dev,KEY_F1 ,1);  input_sync(ts->input_dev);///KEY_F1 */
        ...
        short_pressed = 0; long_pressed = 0;
                ...
                if (timecnt != 1  && jiffies_to_msecs (jiffies - time_prev5) < 50 )  {press_dou = 1;   /// key_released=1; 
///printk("===========%d %s====\n",__LINE__,__FUNCTION__);
return ;}
...
if(press_once==40)
   ...
   ///                     input_report_key(ts->input_dev, KEY_POWER,1);
      ...
      ////                  input_sync(input_dev_g);
                        ...
                        key_released = 0;
                        press_once = 0;
///input_report_key(input_dev_g, KEY_POWER,0);input_sync(input_dev_g);
                        printk("====1222331111111lllllllllllllllllllllllllllllllllllllll=======%d %s====\n",__LINE__,__FUNCTION__);
                        long_pressed = 1; long_press_time =  jiffies;  is_menu_long_pressed = 1;
 ...
 ///    input_sync(ts->input_dev);
	...
	if( 220 < y && y <300 )
	{	/// printk("===========%d %s====\n",__LINE__,__FUNCTION__);
		if (timecnt != 1  && jiffies_to_msecs (jiffies - time_prev) < 400 ) return ;///printk("===========%d %s====\n",__LINE__,__FUNCTION__);
		if(is_long_pressed == 1){
					report_mute_key(1);
					report_mute_key(0);is_long_pressed = 0;	
		}else{			
			input_report_key(ts->input_dev, KEY_VOLUMEDOWN_NWD, 1); 				input_sync(ts->input_dev);	}
		key_pressed= 3;
	}
	if( 0 < y && y <110 ) ////HOME ,6.5INCH ,252
	{	 
		if (timecnt != 1  && jiffies_to_msecs (jiffies - time_prev) < 400 ) return ;		
	...
	{	printk("===========%d %s====\n",__LINE__,__FUNCTION__);
		...
		if(is_long_pressed == 1){
			...
			input_report_key(ts->input_dev, KEY_VOLUMEUP_NWD, 1); //input_report_key(ts->input_dev, KEY_VOLUMEUP_NWD, 0);
		input_sync(ts->input_dev);	
			nwd_dbg_tp();key_pressed= 4;
		}
	}
	timecnt++;
    }
  if( sensor_id_g==0 && y>625) ///6.8INCH ,FOLLOWING 6 KEYS
    {  ///    #define GTP_KEY_TAB  {KEY_F1,  KEY_BACK,KEY_HOME,KEY_VOLUMEDOWN_NWD,KEY_VOLUMEUP_NWD,KEY_DISPLAYTOGGLE_NWD}
	int key_value = 1; static int timecnt=1; 
	machine_type=712;
	/// printk("===========%d %s====\n",__LINE__,__FUNCTION__);
	...
	///if(0) ///( 410< y && y <530 ) ///6.5INCH,PWR
	if( 875 < x && x <1000 ) ///mute sound,6.8INCH,JIANCHANG
{
...
if(press_once==40)
                {
                     input_report_key(ts->input_dev, 242,1);
                       /// input_report_key(input_dev_g, 250,1);
                     input_sync(input_dev_g);
                        ///long_key_to1();
                                        ///printk("===========%d %s====\n",__LINE__,__FUNCTION__);
                        ...
                        key_released = 0;
                        press_once = 0;
			input_report_key(input_dev_g, 242,0);input_sync(input_dev_g);
                        printk("====1222331111111lllllllllllllllllllllllllllllllllllllll=======%d %s====\n",__LINE__,__FUNCTION__);
                        long_pressed = 1; long_press_time =  jiffies;  is_menu_long_pressed = 1;
 ...
 ///    input_sync(ts->input_dev);
	...
	if( 200 < x && x <300 ) ////HOME ,6.5INCH ,252
	{	 
		if (timecnt != 1  && jiffies_to_msecs (jiffies - time_prev) < 400 ) return ;		
		printk("===========%d %s====\n",__LINE__,__FUNCTION__);key_pressed= 2;
		///input_report_key(ts->input_dev, KEY_HOME, 1); ///nwd_dbg_tp();
	...
	if(0)///if( 875 < x && x <1000 ) ///mute sound,6.8INCH,JIANCHANG
	{	printk("===========%d %s====\n",__LINE__,__FUNCTION__);
		if (timecnt != 1  && jiffies_to_msecs (jiffies - time_prev) < 400 ) return ;printk("===========%d %s====\n",__LINE__,__FUNCTION__);
		if(is_long_pressed == 1){
					report_mute_key(1);
			...
			input_report_key(ts->input_dev, 251, 1); //input_report_key(ts->input_dev, KEY_VOLUMEUP_NWD, 0);
		input_sync(ts->input_dev);	
			nwd_dbg_tp();key_pressed= 6;
		}
	}
	if( 380 < x && x <480 ) ///NAVI,6.8INCH,JIANCHANG
	{	printk("===========%d %s====\n",__LINE__,__FUNCTION__);
		if (timecnt != 1  && jiffies_to_msecs (jiffies - time_prev) < 400 ) return ;printk("===========%d %s====\n",__LINE__,__FUNCTION__);
		if(is_long_pressed == 1){
					report_mute_key(1);
			...
			input_report_key(ts->input_dev, 254, 1); //input_report_key(ts->input_dev, KEY_VOLUMEUP_NWD, 0);
		input_sync(ts->input_dev);	
			nwd_dbg_tp();key_pressed= 7;
		}
	}
	timecnt++;
    }
...
static void gtp_touch_up(struct goodix_ts_data* ts, s32 id)
{
	...
	if(key_pressed==4){
		if(is_long_pressed == 0){
			input_report_key(ts->input_dev, KEY_VOLUMEUP_NWD, 0);	input_sync(ts->input_dev);key_pressed= 9;}
		return;
	}
	if(key_pressed==3){
		if(is_long_pressed == 0){
			input_report_key(ts->input_dev, KEY_VOLUMEDOWN_NWD, 0);	input_sync(ts->input_dev);key_pressed= 9;
		}
		return;
	}
	if(key_pressed==0){
		if(is_long_pressed == 0){
			if(is_menu_long_pressed != 1){
				///input_report_key(ts->input_dev,KEY_F1 ,0); input_sync(ts->input_dev);
				///task_key_to0();
			}key_pressed= 9;
		}
		return;
	...
	if(key_pressed==1){
		if(is_long_pressed == 0){
///input_report_key(ts->input_dev, KEY_BACK, 0);	input_sync(ts->input_dev);key_pressed= 9;
   input_report_key(ts->input_dev, 253, 0);	input_sync(ts->input_dev);key_pressed= 9;
		}
		return;
	}
	...
	if(key_pressed==6){
		if(is_long_pressed == 0){
///input_report_key(ts->input_dev, KEY_BACK, 0);	input_sync(ts->input_dev);key_pressed= 9;
   input_report_key(ts->input_dev, 251, 0);	input_sync(ts->input_dev);key_pressed= 9;
		}
		return;
	}
	if(key_pressed==7){
		if(is_long_pressed == 0){
///input_report_key(ts->input_dev, KEY_BACK, 0);	input_sync(ts->input_dev);key_pressed= 9;
   input_report_key(ts->input_dev, 254, 0);	input_sync(ts->input_dev);key_pressed= 9;
		}
		return;
	}
	if(key_pressed==5)
...
if(machine_type==688){
	printk("===========%d %s==long_pressed%d==\n",__LINE__,__FUNCTION__,machine_type);
	    if( long_pressed ==1)
	{
		input_report_key(ts->input_dev, POWEROFFKEYFUN,0); 	input_sync(ts->input_dev);
		long_pressed = 0;
	}
	if( short_pressed == 1)
	{
	
		input_report_key(ts->input_dev, MUTE_SOUND_FUN,0);	input_sync(ts->input_dev);
		short_pressed = 0;
	}
...
if(machine_type==712){
	///printk("===========%d %s==long_pressed%d==\n",__LINE__,__FUNCTION__,machine_type);
	    if( long_pressed ==1)
	{ 
	///	input_report_key(ts->input_dev,KEY_POER,0); 	input_sync(ts->input_dev);
		long_pressed = 0;
	}
	if( short_pressed == 1)
	{
...
#define POWEROFFKEYFUN  KEY_POWER /// 251
#define MUTE_SOUND_FUN  250
void report_poweroffkey(unsigned char on){                    ///long presss
	if(on){
		input_report_key(input_dev_g, POWEROFFKEYFUN,1);input_sync(input_dev_g);
	}else{
		input_report_key(input_dev_g, POWEROFFKEYFUN,0);input_sync(input_dev_g);
	}
}

void report_mute_key(unsigned char on){                    ///long presss
	if(on){
		input_report_key(input_dev_g, MUTE_SOUND_FUN,1);input_sync(input_dev_g);
	}else{
		input_report_key(input_dev_g, MUTE_SOUND_FUN,0);input_sync(input_dev_g);
	}
}
EXPORT_SYNBOL(report_poweroffkey);
EXPORT_SYNBOL(report_mute_key);
...
static void goodix_ts_work_func(struct work_struct *work)
{
  ...
  /// printk("Driver send config.==============================8=========\n\n\n\n\n");
    ...
    {printk("==========%s %d====key_value & (0x01<<i)=\n",__FUNCTION__,__LINE__);
        ...
        #endif
		printk("==========%s %d====key_value & (0x01<<i):%d==\n",__FUNCTION__,__LINE__,key_value & (0x01<<i));
...
#if GTP_COMPATIBLE_MODE
    if (1)///(CHIP_TYPE_GT9F == ts->chip_type)
    {
        ts->fw_error = 0;
    }
    else
...
nwd_dbg_tp();
    if (0)/*** ((!cfg_info_len[1]) && (!cfg_info_len[2]) && 
    ...
    {	nwd_dbg_tp();
        ...
        {	nwd_dbg_tp();
            ...
            ts->pnl_init_error = 1;nwd_dbg_tp();
            return -1;
        }
...
nwd_dbg_tp();
...
#endif
    {
        ...
        if (ret == SUCCESS)
        {
            ...
            {
                GTP_INFO("111111Ic fixed config with config version(%d, 0x%02X)", opr_buf[0], opr_buf[0]);
              ///  ts->fixed_cfg = 1;
             ///   gtp_get_info(ts);
      /*****/ grp_cfg_version = send_cfg_buf[sensor_id][0];       // backup group config version
                send_cfg_buf[sensor_id][0] = 0x00;
     ...
     GTP_INFO("222222222Ic fixed config with config version(%d, 0x%02X)", opr_buf[0], opr_buf[0]);
                ///return 0;
            }
        }
        else
...
s32 gtp_read_version(struct i2c_client *client, u16* version)
{
    ...
    if (buf[5] == 0x00)
    {
        GTP_INFO("\n\n\n\n\n\nIC Version111111111: %c%c%c_%02x%02x", buf[2], buf[3], buf[4], buf[7], buf[6]);
    }
    else
    {
        if (buf[5] == 'S' || buf[5] == 's')
        ...
        GTP_INFO("\n\n\n\n\n\nIC Versio222222222222caocaocaocaocaocao: %c%c%c%c_%02x%02x", buf[2], buf[3], buf[4], buf[5], buf[7], buf[6]);
    }
    return ret;
}
...
static s8 gtp_request_irq(struct goodix_ts_data *ts)
{
    ...
    ret  = request_irq(ts->client->irq, 
                       goodix_ts_irq_handler,
                                    IRQ_TYPE_EDGE_RISING,///  IRQ_TYPE_EDGE_FALLING,/// irq_table[ts->int_trigger_type],
                       ts->client->name,
                       ts);
    if (ret)
    {
...
static s8 gtp_request_input_dev(struct goodix_ts_data *ts)
{
	...
	input_dev_g = ts->input_dev; ///wj add
    ...
    {    ///printk("================================%d %x=====\n\n\n\n",index,touch_key_array[index]);///mdelay(1000);
        input_set_capability(ts->input_dev, EV_KEY, touch_key_array[index]);  
    }
#endif
  	input_set_capability(ts->input_dev, EV_KEY, POWEROFFKEYFUN);   
  	input_set_capability(ts->input_dev, EV_KEY, MUTE_SOUND_FUN);  
 ...
 static int goodix_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    struct goodix_ts_data *ts;
    u16 version_info;
    GTP_INFO("GTP driver installing...ee");
    GTP_DEBUG_FUNC();
    ...
    GTP_INFO("GTP driver installing...ff");
...
retry_tp:
    ...
    ret = gtp_read_version(client, &version_info); ///goto retry_tp;
    if (ret < 0)
    {
...
static void goodix_ts_late_resume(struct early_suspend *h)
{
    struct goodix_ts_data *ts;
    s8 ret = -1; 
	is_suspend=0;is_resume_nwd=0;
    ts = container_of(h, struct goodix_ts_data, early_suspend);
    
printk("\n%d %s ====nwd-sus-re====is_resume_nwd:%d\n",__LINE__,__FUNCTION__,is_resume_nwd);///return ;
    GTP_DEBUG_FUNC();
...
static int __devinit goodix_ts_init(void)
{
    ...
    GTP_DEBUG_FUNC();   
    GTP_INFO("GTP driver installing...aa");
    goodix_wq = create_singlethread_workqueue("goodix_wq");
    if (!goodix_wq)
    {
        GTP_ERROR("Creat workqueue failed.");GTP_INFO("GTP driver installing...bb");
        return -ENOMEM;
    }
#if GTP_ESD_PROTECT
...
#endif
	GTP_INFO("GTP driver installing...cc");
