...
u8 config[GTP_CONFIG_MAX_LENGTH + GTP_ADDR_LENGTH]
	...
	static struct gtp_event prv_event, now_event;
...
#if 0
static int judge_key(int x, int y)
{
	if (x <= 1024)
		return -1;

 	if (y <= 100 && y >= 70)                         //0b10 and 0b100  -----short mute, long power screen
		return KEY_DISPLAYTOGGLE_NWD;

	if (y <= 231 && y >= 181)                       //0b1000
		return KEY_HOME;

	if (y <= 365 && y >= 312)                      //0x10000
		return KEY_BACK;

	if (y <= 475 && y >= 430)                       //0x100000
		return KEY_VOLUMEUP_NWD;

	if (y <= 595 && y >= 555)                      //0x1000000
		return KEY_VOLUMEDOWN_NWD;
	else
		return -2;
...
static int judge_key(int x, int y)
{
	if (x <= POS_X_KEY_EDGE)
		return -1;
#ifdef POWER_KEY_CENTER
 	if (y <= (POWER_KEY_CENTER + RANGE) && y >= (POWER_KEY_CENTER - RANGE))                         //0b10 and 0b100  -----short mute, long power screen
		return KEY_DISPLAYTOGGLE_NWD;
#endif
#ifdef HOME_KEY_CENTER
	if (y <= (HOME_KEY_CENTER + RANGE) && y >= (HOME_KEY_CENTER - RANGE))                       //0b1000
		return KEY_HOME;
#endif
#ifdef BACK_KEY_CENTER
	if (y <= (BACK_KEY_CENTER + RANGE) && y >= (BACK_KEY_CENTER - RANGE))                     //0x10000
		return KEY_BACK;
#endif
#ifdef VOLUM_UP_KEY_CENTER
	if (y <= (VOLUM_UP_KEY_CENTER + RANGE) && y >= ((VOLUM_UP_KEY_CENTER - RANGE)))                       //0x100000
		return KEY_VOLUMEUP_NWD;
#endif
#ifdef VOLUM_DOWN_KEY_CENTER
	if (y <= (VOLUM_DOWN_KEY_CENTER + RANGE) && y >= (VOLUM_DOWN_KEY_CENTER - RANGE))                      //0x1000000
		return KEY_VOLUMEDOWN_NWD;
#endif
...
static void key_timer(unsigned long data)
{
	...
	if( prv_event.keyevent == KEY_HOME )
	{
		home_key_to0();
		prv_event.keystate = 0;
	}
	else if(prv_event.keyevent == KEY_DISPLAYTOGGLE_NWD)
	{
		if(!(prv_event.keystate & 0x2))
		{
			input_event(ts->input_dev, EV_KEY, KEY_DISPLAYTOGGLE_NWD, 1);
			input_sync(ts->input_dev);

			input_event(ts->input_dev, EV_KEY, KEY_DISPLAYTOGGLE_NWD, 0);
			input_sync(ts->input_dev);
		}
		prv_event.keystate = 0;
	}
	...
	{
		input_event(ts->input_dev, EV_KEY, prv_event.keyevent, 0);
		input_sync(ts->input_dev);
		prv_event.keystate = 0;
	}
...
static void gtp_touch_down(struct goodix_ts_data* ts,s32 id,s32 x,s32 y,s32 w)
{
...
#if defined(CONFIG_KEWEI_9INCH_GT911_KEY2)
        x = x + (74 * x) / 1016;
        x = 1096 - x;
	...
	if(ret > 0 || (prv_event.keystate == 1))
	{
		if( prv_event.keystate == 0 )
		{
			prv_event.touch_id = id;
			prv_event.down_jiffies = jiffies;
			prv_event.total_jiffies = 0;
			prv_event.keyevent = ret;
			prv_event.keystate = 1;
        	}
		else 
		{
			if( prv_event.keyevent == KEY_HOME )
			...
			{
				prv_event.total_jiffies = jiffies - prv_event.down_jiffies;
				
				if(jiffies_to_msecs(prv_event.total_jiffies) > 2000 && !(prv_event.keystate & 0x2))
				{
					printk("!state0x2 : 2\n");
					input_event(ts->input_dev, EV_KEY, KEY_POWEROFF_NWD, 1);
					input_sync(ts->input_dev);
						
					input_event(ts->input_dev, EV_KEY, KEY_POWEROFF_NWD, 0);
					input_sync(ts->input_dev);
					prv_event.keystate |= 0x2;
				}
			}
			else
			{
				input_event(ts->input_dev, EV_KEY, prv_event.keyevent, 1);
				input_sync(ts->input_dev);
			}
	...
	defined(CONFIG_YOUYANG_TUGUAN_10INCH_GT928) || \
	defined(CONFIG_KEWEI_9INCH_GT911_KEY2) || \
	defined(CONFIG_QIWEN_8INCH_GT911_A) || \
...
#if defined(CONFIG_YOUYANG_8INCH_GT911)||defined(CONFIG_KEWEI_9INCH_GT911_KEY1)||defined(CONFIG_YOUYANG_TUGUAN_10INCH_GT928) 
    input_set_abs_params(ts->input_dev, ABS_MT_POSITION_X, 0, 1010, 0, 0);
    input_set_abs_params(ts->input_dev, ABS_MT_POSITION_Y, 0, 600, 0, 0);
#elif defined(CONFIG_KEWEI_9INCH_GT911_KEY2)||defined(CONFIG_KEWEI_9INCH_GT911_KEY3)
    input_set_abs_params(ts->input_dev, ABS_MT_POSITION_X, 0, 1024, 0, 0);
    input_set_abs_params(ts->input_dev, ABS_MT_POSITION_Y, 0, 600, 0, 0);
#elif defined(CONFIG_QIWEN_8INCH_GT911)||defined (CONFIG_HANGSHENG_10INCH_GT928)
    input_set_abs_params(ts->input_dev, ABS_MT_POSITION_X, 0, 1024, 0, 0);
    input_set_abs_params(ts->input_dev, ABS_MT_POSITION_Y, 0, 600, 0, 0);
#else
...
static ssize_t is_cts_get(struct device *dev,
...
#if GTP_HAVE_TOUCH_KEY
	now_event.touch_id = -1;			
	now_event.down_jiffies = -1;
	now_event.total_jiffies = 0;
	now_event.keyevent = -1;
	now_event.keystate = 0;

	prv_event.touch_id = -1;			
	prv_event.down_jiffies = -1;
	now_event.total_jiffies = 0;
	prv_event.keyevent = -1;
	prv_event.keystate = 0;
#endif
