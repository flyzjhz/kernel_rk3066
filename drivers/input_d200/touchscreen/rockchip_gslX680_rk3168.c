...
static void gsl_ts_init_worker(struct work_struct *work)
{
	gpio_direction_output(initts->reset_gpio, GPIO_HIGH);
    msleep(10);
    gpio_set_value(initts->reset_gpio,GPIO_LOW);
    msleep(10);
    gpio_set_value(initts->reset_gpio,GPIO_HIGH);
    msleep(300);

	init_chip(initts->client);
	check_mem_data(initts->client);
	printk("------------gsl_ts_init_worker\n");
}
