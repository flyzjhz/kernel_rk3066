		...
		unsigned long arg)
{
	...
	for(i = 0; i < rdwr_arg.nmsgs; i++){
		rdwr_pa[i].scl_rate = 400 * 1000;  //wj add
	}
#ifdef CONFIG_PLAT_RK
        for(i = 0; i < rdwr_arg.nmsgs; i++){
	        res = i2c_transfer(client->adapter, &rdwr_pa[i], 1);
///	printk("i2c len=%d  scl_rate=%d udelay=%d....\n",rdwr_pa[i].len,rdwr_pa[i].scl_rate,rdwr_pa[i].udelay);
                if(res < 0)
                        break;
        }
#else
