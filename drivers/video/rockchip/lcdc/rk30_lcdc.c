...
static int win1_open(struct rk30_lcdc_device *lcdc_dev,bool open)
{
	...
	if(likely(lcdc_dev->clk_on))
	{
		...
		if (win_close_flag <= 0)
		{
			lcdc_msk_reg(lcdc_dev, SYS_CTRL1, m_W0_EN, v_W0_EN(0));
			lcdc_msk_reg(lcdc_dev, SYS_CTRL1, m_W2_EN, v_W2_EN(0));
			win_close_flag = 1;
		}
...
int rk30_lcdc_pan_display(struct rk_lcdc_device_driver * dev_drv,int layer_id)
{
	...
	if(dev_drv->num_buf < 3) //3buffer ,no need to  wait for sysn
	{
		...
		timeout = wait_for_completion_timeout(&dev_drv->frame_done,msecs_to_jiffies(dev_drv->cur_screen->ft+50));
...
static int __devinit rk30_lcdc_probe (struct platform_device *pdev)
{
	...
	clk_disable_unused();
	printk("rk30 lcdc==========================wj place the func here \n");
	return 0;
...
///late_initcall(rk30_lcdc_module_init);
