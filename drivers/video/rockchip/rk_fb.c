...
static int rk_fb_open(struct fb_info *info,int user)
{
	...
	/// wj add
    	...
    	if(dev_drv->layer_par[0]->state)   ///rk_fb.h
    		return 0;
	#endif
	///end
...
static int rk_fb_ioctl(struct fb_info *info, unsigned int cmd,unsigned long arg)
{
	...
	///
#ifdef CONFIG_VIDEO_REVERSE_IMAGE
		case RK_FBIOSET_YUV_ADDR_Y:
			fix->smem_start = arg;  //four y
			break;

		case RK_FBIOSET_YUV_ADDR_UV:
			fix->mmio_start = arg;  //four uv
			break;
#endif
///
...
///
#ifdef CONFIG_VIDEO_REVERSE_IMAGE
static int rk_request_reverse_buffer(struct fb_info *fbi,int fb_id){
	...
	res = platform_get_resource_byname(g_fb_pdev, IORESOURCE_MEM, "reverse image buf");
	if (res == NULL){
		dev_err(&g_fb_pdev->dev, "failed to get memory for reverse image buf \n");
		ret = -ENOENT;
	}
	...
	printk("rk_request_reverse_buffer:fb%d:phy:%lx>>vir:%p>>len:0x%x\n",fb_id,fbi->fix.smem_start,fbi->screen_base,
		fbi->fix.smem_len);
...
//		printk("+++++++++++%d %s  %d %d %d %d %d %d  %d++++++++\n",__LINE__,__FUNCTION__,fbi->var.pixclock,fbi->var.left_margin,fbi->var.right_margin,fbi->var.upper_margin,fbi->var.lower_margin,fbi->var.vsync_len,fbi->var.hsync_len);
		fbi->fbops			 = &fb_ops;
...
///
		#ifdef CONFIG_VIDEO_REVERSE_IMAGE
		if(i == 1)
			rk_request_reverse_buffer(fbi,fb_inf->num_fb);
		#endif
///
