...
#define SENSOR_DG(format, ...) printk(KERN_ERR format, ## __VA_ARGS__) ///dprintk(1, format, ## __VA_ARGS__)
...
static struct reginfo sensor_init_data_err[] ={
	{0x32,0x00},///
	{0xfd,0x01},///
	{0x2c,0x00},///
	{0x2d,0x00},///
...
static int sensor_write_array(struct i2c_client *client, struct reginfo *regarray)
{
    ...
    int i = 0;
    u8 value;
	///while((regarray[i].reg != 0xff) || (regarray[i].val != 0xff))///wj mod
while(!(  (regarray[i].reg == 0xff) && (regarray[i].val == 0xff)))
	///while((regarray[i].reg != 0xff) && (regarray[i].val != 0xff))
    {	
/***
if(regarray[i].reg==0x1b ) goto read;
		if(regarray[i].reg==0x36 && regarray[i].val==0x20) goto read;
	if(regarray[i].reg==0x1b && regarray[i].val==0x3a) goto read;
	if(regarray[i].reg==0x3 && regarray[i].val==0x3) goto read;
	if(regarray[i].reg==0x4 && regarray[i].val==0x66) goto read;
	if(regarray[i].reg==0x1b && regarray[i].val==0x96) goto read;
	if(regarray[i].reg==0x28 && regarray[i].val==0xce) goto read;
	if(regarray[i].reg==0x29 && regarray[i].val==0x8a) goto read;

	if(regarray[i].reg==0x8b && regarray[i].val==0xab) goto read;
	if(regarray[i].reg==0x1b && regarray[i].val==0x80) goto read;
	if(regarray[i].reg==0x34 && regarray[i].val==0x7f) goto read;
	if(regarray[i].reg==0x35 && regarray[i].val==0x40) goto read;
	if(regarray[i].reg==0x36 && regarray[i].val==0x0) goto read;
*/
	...
	if(regarray[i].reg==0xfd) mdelay(30);
read:	sensor_read(client,regarray[i].reg,&value);
	if(regarray[i].val != value)
	{  SENSOR_DG("%s reg[0x%x] check err wjdel err,writte :0x%x  read:0x%x\n",__FUNCTION__,regarray[i].reg,regarray[i].val,value);}
	///else        SENSOR_DG("%s reg[0x%x] check ok wj,writte :0x%x  read:0x%x\n",__FUNCTION__,regarray[i].reg,regarray[i].val,value);
		i++;
    }
...
static int sensor_check_array(struct i2c_client *client, struct reginfo *regarray)
{
    ...
    while((regarray[i].reg != 0xff) || (regarray[i].val != 0xff))
    {
        ...
        }else        SENSOR_DG("%s reg[0x%x] check ok,writte :0x%x  read:0x%x\n",__FUNCTION__,regarray[i].reg,regarray[i].val,value);
	i++;///wj add
...
rk30_mux_api_set(GPIO1A0_UART0SIN_NAME,0);
		...
		{gpio_set_value(RK30_PIN1_PA0, 1);mdelay(15);}
 {	gpio_set_value(RK30_PIN1_PA0, 0);mdelay(15);}
...
gpio_set_value(RK30_PIN4_PB7, 1);mdelay(10);
	...
	////sensor_check_array(client, sensor_init_data);///wj
/**
	mdelay(3000);
   ret = sensor_write_array(client, sensor_init_data_err);
    if (ret != 0)
    {
        SENSOR_TR("error: %s initial failed\n",SENSOR_NAME_STRING());
        goto sensor_INIT_ERR;
    }
	sensor_check_array(client, sensor_init_data_err);///wj add 
*/
...
retry_iic:
{
    ...
    if (ret != 0) {
        SENSOR_TR("%s jjread chip id high byte failed\n",SENSOR_NAME_STRING());
        ret = -ENODEV;
        goto retry_iic;///
    }
    ...
    if (pid == SENSOR_ID) {
        ...
        SENSOR_TR("error: %s mismatched  fae code pid = 0x%x\n", SENSOR_NAME_STRING(), pid);
        ret = -ENODEV;
        goto sensor_video_probe_err;
    }
