...
printk("=========%s %d ===============\n",__FUNCTION__,__LINE__);
	...
	msg[1].flags = client->flags|I2C_M_RD;///I2C_M_RD;///
	msg[1].buf = buf_val;
	msg[1].len = cnt;
	msg[1].scl_rate = i2c_speed;						 /* ddl@rock-chips.com : 100kHz */
	msg[1].read_type = 2;	
	///msg[1].udelay = 5; ///WJ ADD						  /* fpga i2c:0==I2C_NO_STOP : direct use number not enum for don't want include spi_fpga.h */
