...
#define i2c_smbus_write_byte_data_noop(x,y,z) 0
#define i2c_smbus_read_byte_data_noop(x,y) 0
...
static int __adv7180_status(struct i2c_client *client, u32 *status,
	v4l2_std_id *std)
{
	int status1 = i2c_smbus_read_byte_data_noop(client, ADV7180_STATUS1_REG);
...
static int adv7180_v412_init(struct v4l2_subdev *sd, u32 val)
{
	...
	ret = i2c_smbus_write_byte_data_noop(client, ADV7180_INPUT_CONTROL_REG,
		ADV7180_INPUT_CONTROL_AD_PAL_BG_NTSC_J_SECAM_PED);
	if (ret < 0)
		goto err_unreg_subdev;

	ret = i2c_smbus_write_byte_data_noop(client, ADV7180_AUTODETECT_ENABLE_REG,
		ADV7180_AUTODETECT_DEFAULT);
	if (ret < 0)
		goto err_unreg_subdev;

	/* ITU-R BT.656-4 compatible */
	ret = i2c_smbus_write_byte_data_noop(client,
...
static const struct adv7180_picsize adv7180_pic_sizes[] = {
	...
	{720, 480},
	//NTSC
	#if defined(CONFIG_ARCH_RKPX2) || defined(CONFIG_ARCH_RK30)
	{720, 506},
	#else
	{720, 480},
	#endif
...
static void adv7180_work(struct work_struct *work)
{
	...
	i2c_smbus_write_byte_data_noop(client, ADV7180_ADI_CTRL_REG,
		ADV7180_ADI_CTRL_IRQ_SPACE);
	isr3 = i2c_smbus_read_byte_data_noop(client, ADV7180_ISR3_ADI);
	/* clear */
	i2c_smbus_write_byte_data_noop(client, ADV7180_ICR3_ADI, isr3);
	i2c_smbus_write_byte_data_noop(client, ADV7180_ADI_CTRL_REG, 0);
...
static ssize_t store_dbg(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    i2c_smbus_write_byte_data_noop(client, reg, val);
    printk("write 0x%02x = 0x%02x, ret = 0x%02x\n", reg, val, i2c_smbus_read_byte_data_noop(client, reg));
			...
			const struct i2c_device_id *id)
{
	...
	nwd();
	/***
	rk30_mux_api_set(GPIO1B4_CIF0DATA0_NAME, GPIO1B_GPIO1B4);
	ret = gpio_request(RK30_PIN1_PB4, "ADV_INT");
	if (ret != 0)
	{
		gpio_free(RK30_PIN1_PB4);
		printk(KERN_ERR "request ADV_INT en pin fail!\n");
		return -1;
	}
	gpio_direction_input(RK30_PIN1_PB4);*/
	
	nwd();
	//state->irq = gpio_to_irq(RK30_PIN1_PB4);//client->irq;
	...
	ret = i2c_smbus_read_byte_data_noop(client, ADV7180_IDENT_REG);
	/*
	...
	printk("%s %d\n", __func__, __LINE__);
       ...
       ret = i2c_smbus_write_byte_data_noop(client, 0x1d, 0x00); /* 27M/28M crystal */
	nwd();
	...
	printk("%s %d\n", __func__, __LINE__);
		...
		ret = i2c_smbus_write_byte_data_noop(client, ADV7180_ADI_CTRL_REG,
			ADV7180_ADI_CTRL_IRQ_SPACE);
		if (ret < 0)
			goto err_unreg_subdev;
	printk("%s %d\n", __func__, __LINE__);
		...
		ret = i2c_smbus_write_byte_data_noop(client, ADV7180_ICONF1_ADI,
			ADV7180_ICONF1_ACTIVE_LOW | ADV7180_ICONF1_PSYNC_ONLY);
		if (ret < 0)
			goto err_unreg_subdev;
	printk("%s %d\n", __func__, __LINE__);

		ret = i2c_smbus_write_byte_data_noop(client, ADV7180_IMR1_ADI, 0);
		if (ret < 0)
			goto err_unreg_subdev;
	printk("%s %d\n", __func__, __LINE__);

		ret = i2c_smbus_write_byte_data_noop(client, ADV7180_IMR2_ADI, 0);
		if (ret < 0)
			goto err_unreg_subdev;
	printk("%s %d\n", __func__, __LINE__);

		/* enable AD change interrupts interrupts */
		ret = i2c_smbus_write_byte_data_noop(client, ADV7180_IMR3_ADI,
			ADV7180_IRQ3_AD_CHANGE);
		if (ret < 0)
			goto err_unreg_subdev;
	printk("%s %d\n", __func__, __LINE__);

		ret = i2c_smbus_write_byte_data_noop(client, ADV7180_IMR4_ADI, 0);
		if (ret < 0)
			goto err_unreg_subdev;
	printk("%s %d\n", __func__, __LINE__);
...
subsys_initcall_sync(adv7180_init);
#else
