    ...
    if (vb) {
		if (CAM_WORKQUEUE_IS_EN()) {
			...
			if (y_addr > (pcdev->vipmem_phybase + pcdev->vipmem_size - pcdev->vipmem_bsize)) {
				RKCAMERA_TR("vipmem for IPP is overflow111! %dx%d -> %dx%d vb_index:%d\n",pcdev->host_width,pcdev->host_height,
...
//    unsigned int cif_fmt_val = read_cif_reg(pcdev->base,CIF_CIF_FOR) | INPUT_MODE_YUV | YUV_INPUT_422| CCIR_INPUT_ORDER_ODD | OUTPUT_420_ORDER_EVEN;
	...
	if(ret == 0) {
		if(stdid == V4L2_STD_NTSC) {
			...
			printk("%s unkown std, set as pal wj //ntsc\n", __FUNCTION__);
			cif_fmt_val |= INPUT_MODE_PAL;////INPUT_MODE_NTSC;
		}
		cif_fmt_val |= (1 << 9);
	}
...
static bool rk_camera_fmt_capturechk(struct v4l2_format *f)
{
	...
	if ((f->fmt.pix.width == 720) && (f->fmt.pix.height == 480)) {
		ret = true;
	} else if ((f->fmt.pix.width == 720) && (f->fmt.pix.height == 625)) {
		ret = true;
   ...
   /* limit to rk29 hardware capabilities */
	...
	if ((mf.width != usr_w) || (mf.height != usr_h)) {
		...
		if (vipmem_is_overflow == false) {
			...
			RKCAMERA_TR("vipmem for IPP is overflow222, This resolution(%dx%d -> %dx%d) is invalidate!\n",mf.width,mf.height,usr_w,usr_h);
            pix->width = mf.width;
            pix->height = mf.height;            
		}
...
#elif defined(CONFIG_ARCH_RKPX2) || defined(CONFIG_ARCH_RK30)
    switch(cif_index){
        case 0:
        {
...
static int rk_camera_probe(struct platform_device *pdev)
{
    ...
    if (meminfo_ptr->vbase == NULL) {
        ...
        if ((meminfo_ptr->start == meminfo_ptrr->start)
            ...
            if (!request_mem_region(meminfo_ptr->start,meminfo_ptr->size,"rk29_vipmem")) {
                err = -EBUSY;
                RKCAMERA_TR("%s(%d): request_mem_region(start:0x%x size:0x%x) failed \n",__FUNCTION__,__LINE__, pcdev->pdata->meminfo.start,pcdev->pdata->meminfo.size);nwd();
    ...
    sema_init(&pcdev->zoominfo.sem,1);nwd();
	...
	nwd();
	reverse_pcdev = pcdev;
	///rk_reverse_image_main(void *arg
	kthread_run(rk_reverse_image_main,NULL,"reverse_image_thread");
#endif
    return 0;
