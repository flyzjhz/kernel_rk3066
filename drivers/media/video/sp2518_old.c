...
#define CONFIG_SENSOR_I2C_SPEED     50000 ///100000	///250000       /* Hz */
...
static struct reginfo sensor_init_data[] =
{
	...
	{0x31,0x00},//mirror upside down ///err
	{0x44,0x03},
	{0x6f,0x00},
	{0xa0,0x04},
	{0x5f,0x01},
...
static int sensor_read(struct i2c_client *client, u8 reg, u8 *val)
{
 ...
 ///   msg[0].read_type = 2;   /* fpga i2c:0==I2C_NO_STOP : direct use number not enum for don't want include spi_fpga.h */
    ...
    msg[1].flags = I2C_M_RD;///client->flags|I2C_M_RD;
    msg[1].buf = buf;
    msg[1].len = 1;
...
static int sensor_write_array(struct i2c_client *client, struct reginfo *regarray)
{
    ...
    {
    re_wr:     err = sensor_write(client, regarray[i].reg, regarray[i].val);
        if (err != 0)
        {
...
for(i=0;i<sizeof(*regarray) / 2;i++)
  {
	 ...
	 if(regarray[i].val != value)
	 {
	  SENSOR_DG("%s reg[0x%x] check err,writte :0x%x  read:0x%x\n",__FUNCTION__,regarray[i].reg,regarray[i].val,value);
		  ...
		  SENSOR_DG("%s reg[0x%x] check ok wj,writte :0x%x  read:0x%x\n",__FUNCTION__,regarray[i].reg,regarray[i].val,value);
...
static int sensor_init(struct v4l2_subdev *sd, u32 val)
{
 ...
 ret = sensor_write(client, 0x12, 0x80);
	...
	/***sensor_check_array(client, sensor_init_data);///wj add 
...
sensor_write_array(client, sensor_init_data); ///wj add
///sensor_check_array(client, sensor_init_data);///wj add 
#if 1
    if (pid == SENSOR_ID) {
