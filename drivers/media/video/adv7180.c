...
struct adv7180_state {
	...
	bool			autodetect;
	struct soc_camera_device *icd; /* added by nt */
};
...
static int adv7180_set_mbus_fmt(struct v4l2_subdev *sd, struct v4l2_mbus_framefmt *mf)
{
	...
	printk("heyu %s %d mf->width=%d,mf->height=%d, index=%d\n", __func__, __LINE__,mf->width,mf->height, index);
	return 0;
}
...
static int adv7180_try_mbus_fmt(struct v4l2_subdev *sd, struct v4l2_mbus_framefmt *mf)
{
	...
	//printk("%s %d  mf->width = %d,mf->height = %d  \n", __func__, __LINE__,mf->width,mf->height);
...
static void adv7180_work(struct work_struct *work)
{
	...
	if (ADV7180_IRQ3_AD_CHANGE && state->autodetect)
		__adv7180_status(client, NULL, &state->curr_norm);
	mutex_unlock(&state->mutex);
