...
static int rk_sensor_pwrseq(struct device *dev,int powerup_sequence, int on, int mclk_rate)
{
    ...
    for (i=0; i<8; i++) {
		...
		///gpio_setting_tst(  RK30_PIN6_PA7,"high","cam pwr");
#endif 
		///	ret = 0;
...
/***
		rk30_mux_api_set(GPIO6A7_TESTCLOCKOUT_NAME,0);
		gpio_setting_tst(  RK30_PIN6_PA7,"GPIO6A7 high","cam pwr");mdelay(300);
    	 { rk_sensor_ioctrl(dev,Cam_Mclk, 1); printk("%d\n",__LINE__);}

			rk30_mux_api_set(GPIO4B7_SPI0CSN1_NAME,0);
			gpio_setting_tst(  RK30_PIN4_PB7,"GPIO4B7 low","cam rst");mdelay(300);
		...
		gpio_set_value(RK30_PIN1_PA0, 1);mdelay(20);
		gpio_set_value(RK30_PIN1_PA0, 0);mdelay(20);


			///gpio_set_value(RK30_PIN4_PB7, 0);mdelay(30);
			gpio_set_value(RK30_PIN4_PB7, 1);mdelay(30);
*/
...
///gpio_set_value(RK30_PIN4_PB7, 1);mdelay(30);
			...
			///gpio_set_value(RK30_PIN4_PB7, 0);mdelay(30);
			///gpio_set_value(RK30_PIN4_PB7, 1);mdelay(30);
		/***	gpio_set_value(RK30_PIN4_PB7, 0);mdelay(3000);
			gpio_set_value(RK30_PIN4_PB7, 1);mdelay(3000);
	gpio_set_value(RK30_PIN4_PB7, 0);mdelay(3000);
			gpio_set_value(RK30_PIN4_PB7, 1);mdelay(3000);*/


		///gpio_set_value(RK30_PIN1_PA0, 1);mdelay(3000);
		///ret = 0;

/***
	rk30_mux_api_set(GPIO4C5_SMCDATA5_TRACEDATA5_NAME, GPIO4C_GPIO4C5); ///RST
gpio_setting_tst( RK30_PIN4_PC5,"GPIO4C5 high","tp rst");mdelay(300);
	gpio_set_value(RK30_PIN4_PC5, 1);mdelay(30);
			gpio_set_value(RK30_PIN4_PC5, 0);mdelay(30);
			gpio_set_value(RK30_PIN4_PC5, 1);mdelay(30);*/
#endif
                break;
            }
...
#else /*
		rk30_mux_api_set(GPIO1A0_UART0SIN_NAME,0);
		gpio_setting_tst(  RK30_PIN1_PA0,"high","cam pwdn");
		gpio_set_value(RK30_PIN1_PA0, 0);*/
#endif
			///ret = 0;
                break;
            }
