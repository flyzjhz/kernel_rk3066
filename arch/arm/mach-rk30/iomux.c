...
/*MUX_CFG(GPIO6B6_TESTCLOCKOUT_NAME,			GPIO6B,   12,	 1,   0,	DEFAULT) 
MUX_CFG(GPIO6B5_TESTCLOCKOUT_NAME,			GPIO6B,   10,	 1,   0,	DEFAULT)
MUX_CFG(GPIO6B4_TESTCLOCKOUT_NAME,			GPIO6B,   8,	 1,   0,	DEFAULT)*/
MUX_CFG(GPIO6B3_TESTCLOCKOUT_NAME,			GPIO6B,   6,	 1,   0,	DEFAULT)
/***MUX_CFG(GPIO6B2_TESTCLOCKOUT_NAME,			GPIO6B,   4,	 1,   0,	DEFAULT)
MUX_CFG(GPIO6B1_TESTCLOCKOUT_NAME,			GPIO6B,   2,	 1,   0,	DEFAULT)*/
MUX_CFG(GPIO6B0_TESTCLOCKOUT_NAME,			GPIO6B,   0,	 1,   0,	INITIAL) /*** LCD_EN ,WJ ADD */
};     
...
void rk30_mux_set2(struct mux_config *cfg)
{
	int regValue = 0;
	int mask;
	...
	regValue |=(cfg->mode<<cfg->offset);
#if     1 
	printk("====%s:physreg=0x%x  :reg=0x%p,Value=0x%x,mask=0x%x\n",__FUNCTION__,virt_to_phys(cfg->mux_reg),cfg->mux_reg,regValue,mask);
#endif
	writel_relaxed(regValue,cfg->mux_reg);
printk(" ===IOMUXNWD=addr %x = %x",virt_to_phys(cfg->mux_reg),readl(cfg->mux_reg));
	dsb();
...
int __init rk30_iomux_init(void)
{
	...
	rk30_mux_api_set(GPIO1B3_CIF0CLKOUT_NAME,GPIO1B_CIF0_CLKOUT); ///jj add
...
/***
	ret = gpio_request( RK30_PIN3_PA1, "sddddd");
	if (ret != 0) {
		gpio_free(RK30_PIN3_PA1);
		printk(">>>>>> IRDA_IRQ_PIN gpio_request err \n ");
	}
	gpio_pull_updown(RK30_PIN3_PA1, 2);
	gpio_free(RK30_PIN3_PA1);

	ret = gpio_request( RK30_PIN3_PA0, "sdddeeed");
	if (ret != 0) {
		gpio_free(RK30_PIN3_PA0);
		printk(">>>>>> IRDA_IRQ_PIN gpio_request err \n ");
	}
	gpio_pull_updown(RK30_PIN3_PA0,2);
	gpio_free(RK30_PIN3_PA0);*/
#endif
...
void rk30_mux_api_set2(char *name, unsigned int mode)
{
  int i;
        if (!name) {
                return;
...
printk("\n==========IOMUXWJ %s\n",name);
	for(i=0;i<ARRAY_SIZE(rk30_muxs);i++)
	{
		if (!strcmp(rk30_muxs[i].name, name))
		{
		    rk30_muxs[i].premode = rk30_muxs[i].mode;
			rk30_muxs[i].mode = mode;
			rk30_mux_set2(&rk30_muxs[i]);	
			break;			
		}
	}
}
EXPORT_SYMBOL(rk30_mux_api_set2);
