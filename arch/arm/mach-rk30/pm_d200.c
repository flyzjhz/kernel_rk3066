...
static void ddr_testmode(void)
{
	...
	if (ddr_debug == 1) {
		...
		///for (;;) 
		{
			...
			sram_printhex(nMHz);
			sram_printch(' '); nMHz=0x000000f8;
			nMHz = ddr_change_freq(nMHz);	 ///mdelay(2000);		
			sram_printhex(n++);
			sram_printch(' ');
...
{	dbg_nwd();
	...
	dbg_nwd();
	rk_pm_soc_prepare();dbg_nwd();
	
	printk(KERN_DEBUG "pm: ");
	pm_log = true;
	sram_log_reset();dbg_nwd();
	sram_printch('0');dbg_nwd();
	is_pwr_rpt=3;
	RK_SOC_PM_CTR_FUN(NO_PD,rk_pm_soc_pd_suspend);dbg_nwd();

	sram_printch('1');dbg_nwd();
	local_fiq_disable();
	
	RK_SOC_PM_CTR_FUN(NO_CLK_GATING,rk_pm_soc_clk_gating_first);dbg_nwd();

	sram_printch('2');
	RK_SOC_PM_CTR_FUN(NO_PLL,rk_pm_soc_pll_suspend);dbg_nwd();
	...
	interface_ctr_reg_pread();dbg_nwd();
	...
	RK_SOC_PM_CTR_FUN(NO_VOLT,rk30_pwm_resume_voltage_set);dbg_nwd();
	...
	RK_SOC_PM_CTR_FUN(NO_CLK_GATING,rk_pm_soc_clk_ungating_first);dbg_nwd();
	
	local_fiq_enable();
	sram_printch('1');

	RK_SOC_PM_CTR_FUN(NO_PD,rk_pm_soc_pd_resume);dbg_nwd();
	
	sram_printch('0');dbg_nwd();
	pm_log = false;
	printk(KERN_CONT "\n");dbg_nwd();
	sram_printch('\n');
	rk_pm_soc_finsh();dbg_nwd();is_resume_nwd=1;
	return 0;
}
