...
struct snd_soc_codec *init_codec=NULL;
...
static int es8323_mute(struct snd_soc_dai *dai, int mute)
{
  ...
  ///snd_soc_write(codec, ES8323_DACCONTROL3, 0x06);//0xe6);
	snd_soc_write(codec, ES8323_DACCONTROL3, 0xe4);//0xe6);
	...
	else
	{
		...
		snd_soc_write(codec, ES8323_DACCONTROL3, 0xe0);//0xe2);
...
static void code_init_work(struct work_struct *work)
{
	struct snd_soc_codec *codec=init_codec;
	struct snd_soc_dapm_context *dapm = &codec->dapm;

	nwd();
    snd_soc_write(codec, 0x35  , 0xa0); 
    snd_soc_write(codec, 0x36  , 0x08); //for 1.8V VDD
	snd_soc_write(codec, 0x02,0xf3);nwd();
	snd_soc_write(codec, 0x2B,0x80);///printk("++++++++++val%x\n",snd_soc_read(codec, 0x2B));
	snd_soc_write(codec, 0x08,0x00);  ///printk("++++++++++val%x\n",snd_soc_read(codec, 0x08)); //ES8388 salve  
	snd_soc_write(codec, 0x00,0x32);   //
	...
	snd_soc_write(codec, 0x09,0x88);   ///printk("++++++++++val%x\n",snd_soc_read(codec, 0x09)); //ES8388 salve  //ADC L/R PGA =  +24dB
 	nwd();
	//----------------------------------------------------------------------------------------------------------------
	...
	snd_soc_write(codec, 0x17,0x18);   ///printk("++++++++++val%x\n",snd_soc_read(codec, 0x17)); //ES8388 salve  //I2S-16BIT
	snd_soc_write(codec, 0x18,0x02);
	snd_soc_write(codec, 0x1A,0x00);  //DAC VOLUME=0DB
	snd_soc_write(codec, 0x1B,0x00); ///printk("++++++++++val%x\n",snd_soc_read(codec, 0x1B)); //ES8388 salve  
	/*
	...
	*/
	...
	msleep(100);
	snd_soc_write(codec, 0x04,0x0c);  ///printk("++++++++++val%x\n",snd_soc_read(codec, 0x04)); //pdn_ana=0,ibiasgen_pdn=0 
...
static DECLARE_WORK(init_work,code_init_work);
				      ...
				      const struct i2c_device_id *id)
{
	...
	///es8323_class = class_create(NULL, "es8323");
