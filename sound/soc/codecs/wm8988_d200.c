...
static const u16 wm8988_reg[] = {
	...
	0x0050, 0x0050, 0x0000, 0x0000,  /* 36 */
...
struct snd_soc_codec *codec_g;
 void read_vol(void){
	printk("aabb \n");printk("\n");
	int cnt=0;
	for(;cnt<43;cnt++){
		if(cnt%4==0) printk("\n");
		printk("  R%02d %02x:%04x",cnt,cnt,snd_soc_read(codec_g, cnt));	
	}
	///cnt=67;printk("  R%d %02x:%04x",cnt,cnt,snd_soc_read(codec_g, cnt));
	printk("\n");printk("\n");printk("\n");
}
EXPORT_SYMBOL(read_vol);
...
/////8	SND_SOC_DAPM_MICBIAS("Mic Bias", WM8988_PWR1, 1, 0),
	...
	/////8 SND_SOC_DAPM_MUX("Left PGA Mux", WM8988_PWR1, 5, 0,&wm8988_left_pga_controls),
/////8	SND_SOC_DAPM_MUX("Right PGA Mux", WM8988_PWR1, 4, 0,&wm8988_right_pga_controls),
	...
	/////8 SND_SOC_DAPM_ADC("Right ADC", "Right Capture", WM8988_PWR1, 2, 0),
	/////8 SND_SOC_DAPM_ADC("Left ADC", "Left Capture", WM8988_PWR1, 3, 0),
			      ...
			      struct snd_soc_dai *dai)
{
	...
	if (!wm8988->is_startup) {
		...
		snd_soc_write(codec, WM8988_PWR2, 0x19f);
...
/*** wj add
	...
	snd_soc_write(codec, 0x28, 0x017f); 
	snd_soc_write(codec, 0x29, 0x017f);


snd_soc_write(codec, 0xa, 0x01ff); 
	snd_soc_write(codec, 0xb, 0x01ff);*/

	snd_soc_write(codec, 0x1a, 0x01ff); ///
	return 0;
}
...
static int wm8988_mute(struct snd_soc_dai *dai, int mute)
{
	...
	DBG("Enter::%s----%d--mute=%d CAOCAOCAONIDIE 22\n",__FUNCTION__,__LINE__,mute);
...
vol_max()
{
printk("AUDIO max %x \n",snd_soc_read(codec_g,WM8988_LOUT1V ));
printk(" %x \n",snd_soc_read(codec_g,WM8988_ROUT1V ));
printk(" %x \n",snd_soc_read(codec_g,0x28 ));
printk(" %x \n",snd_soc_read(codec_g,0x29 ));
printk(" %x \n",snd_soc_read(codec_g,0xa ));
printk(" %x \n",snd_soc_read(codec_g,0xb ));
/***
snd_soc_write(codec_g, 0x1a,0x0);  ///wj add
	snd_soc_write(codec_g, 0x1b,0);	
	snd_soc_write(codec_g, 0x2e,0x21);  ///wj add
	snd_soc_write(codec_g, 0x2f,0x21);
	snd_soc_write(codec_g, 0x30,0x21); 
	snd_soc_write(codec_g, 0x31,0x21);

printk("AUDIO max %x \n",snd_soc_read(codec_g,0x1a ));
printk(" %x \n",snd_soc_read(codec_g,0x1b ));
printk(" %x \n",snd_soc_read(codec_g,0x2e ));
printk(" %x \n",snd_soc_read(codec_g,0x2f ));
printk(" %x \n",snd_soc_read(codec_g,0x30 ));
printk(" %x \n",snd_soc_read(codec_g,0x31 ));*/
}
EXPORT_SYNBOL(vol_max);
#if 0
read_vol()
{
printk("AUDIO max %x \n",snd_soc_read(codec_g,WM8988_LOUT1V ));
...
{
	DBG("Enter::%s----%d\n",__FUNCTION__,__LINE__);
	is_suspend=1;is_pwr_rpt=3;///
	wm8988_set_bias_level(codec, SND_SOC_BIAS_OFF);
	return 0;
}
void report_pwr_key_resume(void);
extern is_resume_nwd;
static int wm8988_probe2(struct snd_soc_codec *codec);
static int wm8988_resume(struct snd_soc_codec *codec)
{
	...
	u16 *cache = codec->reg_cache;
        ...
        is_suspend=0;
	DBG("Enter::%s----%d is_resume_nwd:%d\n",__FUNCTION__,__LINE__,is_resume_nwd);
	if(is_pwr_rpt!=1){
...
{
	snd_soc_write(codec_g, reg, val);
}
EXPORT_SYMBOL(wm8999_wr);
#define  NO_TA 1
static int wm8988_probe2(struct snd_soc_codec *codec)
{
	struct wm8988_priv *wm8988 = snd_soc_codec_get_drvdata(codec);
...
//	snd_soc_write(codec, WM8988_ROUT1V, 0x017f);
	...
	snd_soc_write(codec, WM8988_LOUT2V, 0x017a);  /*** wj add */
	snd_soc_write(codec, WM8988_ROUT2V, 0x017a);





	snd_soc_write(codec, WM8988_LOUT1V, 0x0179); 
	snd_soc_write(codec, WM8988_ROUT1V, 0x0179);
	
	snd_soc_write(codec, WM8988_LOUT2V, 0x0177);  /*** wj add */
	snd_soc_write(codec, WM8988_ROUT2V, 0x0177);
	...
	snd_soc_write(codec, WM8988_LDAC, 0xfa/*0xff*/);  // Change max by zhansb@110415
	snd_soc_write(codec, WM8988_RDAC, 0xfa/*0x1ff*/);//vol set 

	snd_soc_write(codec, WM8988_LDAC, 0x1ff/*0xff*/);  // Change max by zhansb@110415
	snd_soc_write(codec, WM8988_RDAC, 0x1ff/*0x1ff*/);//vol set 
	...
	snd_soc_write(codec, WM8988_LOUT1V, 0x0100); 
	snd_soc_write(codec, WM8988_ROUT1V, 0x0100);

snd_soc_write(codec,0x1a, snd_soc_read(codec,0X1a)&(0xff9f));///wj add

snd_soc_write(codec,0x19, 0x00fc);///wj add
/***
	snd_soc_write(codec, WM8988_LOUT1V, 0x017f); 
	snd_soc_write(codec, WM8988_ROUT1V, 0x017f);
...
/***/
	...
	///reg = snd_soc_read(codec, 0x1a);
///	snd_soc_write(codec, 0x1a,reg|(1<<6)|(1<<5));  ///for hot
	
	snd_soc_write(codec, 0x1a,0x19f);  ///for hot
...
*/
	snd_soc_write(codec, 0x1a,0x19f);  ///wj,for hot
	return 0;
}
...
wm8988_out1_pwrdn()
{
	snd_soc_write(codec_g, 0x1a,0x19f);  ///wj,for hot
}
EXPORT_SYMBOL(wm8988_out1_pwrdn);
