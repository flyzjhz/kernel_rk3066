...
#endif
#include "power.h"
extern unsigned char is_suspend;
enum {
...
static int lock_stat_sleep( struct wake_lock *lock)
{
	int lock_count = lock->stat.count;
	int expire_count = lock->stat.expire_count;
	ktime_t active_time = ktime_set(0, 0);
...
/*
printf(		     "\"%s\"\t%d\t%d\t%d\t%lld\t%lld\t%lld\t%lld\t%lld\n",
		     lock->name, lock_count, expire_count,
		     lock->stat.wakeup_count, ktime_to_ns(active_time),
		     ktime_to_ns(total_time),
		     ktime_to_ns(prevent_suspend_time), ktime_to_ns(max_time),
...
///if( strcmp(lock->name,"main")!=0)
 ///continue;
///printk("==%d %s  %d",__LINE__,__FUNCTION__,is_suspend);
if( strcmp(lock->name,"PowerManagerService.WakeLocks")!=0  && is_suspend==1  && lock_stat_sleep(lock)==1  && strcmp(lock->name,"main")!=0  )
 { if( strcmp(lock->name,"mmc1_detect")!=0  && strcmp(lock->name,"mmc2_detect")!=0  &&strcmp(lock->name,"mmc0_detect")!=0 )
   continue;
 }	
if(1){
if( strcmp(lock->name,"usb_pcd")==0&& lock_stat_sleep(lock)==1 && is_suspend==1 )
 continue;

//////if( strcmp(lock->name,"PowerManagerService.Display")==0) ///for daodaotong  big 
//////continue;

if( strcmp(lock->name,"AudioOutLock")==0 && lock_stat_sleep(lock)==1 && is_suspend==1  )  ///for daodaotong  big 
continue;
/***/
////if( strcmp(lock->name,"main")==0)
/// continue;
#if 1
/////if( strcmp(lock->name,"unknown_wakeups")==0)
 /////continue;

/***/
///if( strcmp(lock->name,"PowerManagerService.WakeLocks")==0)
  ///continue;

//////if( strcmp(lock->name,"PowerManagerService.Display")==0) ///for daodaotong  big 
 //////continue;

//////if( strcmp(lock->name,"alarm")==0) ///for daodaotong  big 
 /////continue; /*** */     /****/
#endif  

if( strcmp(lock->name,"mmc1_detect")==0 && is_suspend==1  ) ///duan acc,then qucik unsert sd(when using sd) ,
 continue;
if( strcmp(lock->name,"mmc0_detect")==0  && is_suspend==1 ) ///duan acc,then qucik unsert sd(when using sd) ,
 continue;

if( strcmp(lock->name,"mmc2_detect")==0  && is_suspend==1 ) ///duan acc,then qucik unsert sd(when using sd) ,
 continue;
...
/***
if( strcmp(lock->name,"mmc2_detect")==0 && is_suspend==1 )  ///duan acc,then qucik unsert sd(when using sd) ,
{printk("==mmc2=== -1 %d %s  %d",__LINE__,__FUNCTION__,is_suspend);
		lock->expires = jiffies-1;
 continue;
}*/
/***if( strcmp(lock->name,"mmc1_detect")==0&&lock_stat_sleep(lock)==1 && is_suspend==1  ) ///duan acc,then qucik unsert sd(when using sd) ,
 continue;
if( strcmp(lock->name,"mmc0_detect")==0 && lock_stat_sleep(lock)==1 && is_suspend==1 ) ///duan acc,then qucik unsert sd(when using sd) ,
 continue;
if( strcmp(lock->name,"mmc2_detect")==0 && lock_stat_sleep(lock)==1 && is_suspend==1 )  ///duan acc,then qucik unsert sd(when using sd) ,
 continue;*/
}

///if( strcmp(lock->name,"AudioOutLock")==0)
 ////continue;
}
		if (lock->flags & WAKE_LOCK_AUTO_EXPIRE) {
			long timeout = lock->expires - jiffies;
